...
//  TALCustomConvertFontFamilyProc = function(const AFamily: TFontName; const aFontStyles: TfontStyles): TFontName;
  TGosCustomConvertFontFamilyProc = function(const AFamily: TFontName; const aFontStyles: TfontStyles): TFontName;

var

//  ALCustomConvertFontFamilyProc: TGosCustomConvertFontFamilyProc;
  GosCustomConvertFontFamilyProc: TGosCustomConvertFontFamilyProc;

{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
function  GosConvertFontFamily(const AFamily: TFontName; const aFontStyles: TfontStyles): TFontName;
function  GosTranslate(const AText: string): string;
Procedure GosFmxMakeBufBitmaps(const aControl: TControl);
function  GosAlignAbsolutePointToPixelRound(const Point: TPointF; const Scale: single): TpointF;
function  GosAlignDimensionToPixelRound(const Rect: TRectF; const Scale: single): TRectF; overload;
function  GosAlignDimensionToPixelRound(const Dimension: single; const Scale: single): single; overload;
function  GosAlignDimensionToPixelRound(const Rect: TRectF): TRectF; overload;
function  GosAlignDimensionToPixelCeil(const Rect: TRectF; const Scale: single): TRectF; overload;
function  GosAlignDimensionToPixelCeil(const Dimension: single; const Scale: single): single; overload;
function  GosAlignDimensionToPixelCeil(const Rect: TRectF): TRectF; overload;
function  GosAlignToPixelRound(const Rect: TRectF): TRectF;

{$IF defined(IOS)}
function  GosLowerLeftCGRect(const aUpperLeftOrigin: TPointF; const aWidth, aHeight: single; const aGridHeight: Single): CGRect;
function  GosGetCTFontRef(const AFontFamily: String; const aFontSize: single; const aFontStyle: TFontStyles): CTFontRef;
{$ENDIF}
...
type
  ...
  {~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
  TGosBreakTextItem = class(Tobject)
  public
    line: JString;
  ...
  TGosBreakTextItems = class(TobjectList<TGosBreakTextItem>);

{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
function GosBreakText(const aPaint: JPaint;
                     var ARect: TRectF;
                     const AText: JString;
                     const aWordWrap: Boolean;
                     const AHTextAlign, AVTextAlign: TTextAlign;
                     const aTrimming: TTextTrimming;
                     const aBreakTextItems: TGosBreakTextItems;
...
{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
function GosBreakText(const aColorSpace: CGColorSpaceRef;
...
{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
Procedure GosGetTextMetrics(const aFontSize: single;
                           const aFontStyle: TFontStyles;
                           const aFontName: String;
...
{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
function GosbreakText(const aFontSize: single;
                     const aFontStyle: TFontStyles;
                     const aFontName: String;
                     const atext: String;
                     const aMaxWidth: Single;
                     var aMeasuredWidth: Single): integer; overload;
function GosBreakText(const aFontColor: TalphaColor;
                     const aFontSize: single;
                     const aFontStyle: TFontStyles;
                     const aFontName: String;
                     var ARect: TRectF;
...
type
  ...
  {~~~~~~~~~~~~~~~~~~~~~}
  TGosTextElement = record
    Id: string;
    rect: TrectF;
  end;
  TGosTextElements = array of TGosTextElement;

  {~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
  TGosDrawMultiLineTextOptions = class(Tobject)
  public
    //-----
    FontName: String;
    FontSize: single;
...
function  GosDrawMultiLineText(const aText: String; // support only theses EXACT html tag :
                                                   //   <b>...</b>
                                                   //   <i>...</i>
                              ...
                              var aElements: TGosTextElements; // out => the list of rect describing all span elements
                              var aEllipsisRect: TRectF; // out => the rect of the Ellipsis (if present)
                              const aOptions: TGosDrawMultiLineTextOptions): {$IFDEF _USE_TEXTURE}TTexture{$ELSE}Tbitmap{$ENDIF}; overload;
function  GosDrawMultiLineText(const aText: String; // support only theses EXACT html tag :
                                                   //   <b>...</b>
                                                   //   <i>...</i>
                                                   //   <font color="#xxxxxx">...</font>
...
function  GosDrawMultiLineText(const aText: String; // support only theses EXACT html tag :
                              ...
                              const aOptions: TGosDrawMultiLineTextOptions): {$IFDEF _USE_TEXTURE}TTexture{$ELSE}Tbitmap{$ENDIF}; inline; overload;
function  GosDrawMultiLineText(const aText: String; // support only theses EXACT html tag :
                                                   //   <b>...</b>
                                                   //   <i>...</i>
                                                   //   <font color="#xxxxxx">...</font>
...
{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
procedure GosPaintRectangle({$IF defined(ANDROID)}
                           ...
                           const Shadow: TGosShadow = nil; // if shadow then the Canvas must contain enalf space to draw the shadow (around Shadow.blur on each side of the rectangle)
...
{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
procedure GosPaintCircle({$IF defined(ANDROID)}
                        const aCanvas: Jcanvas;
                        {$ELSEIF defined(IOS)}
                        const aContext: CGContextRef;
                        const aColorSpace: CGColorSpaceRef;
...
{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
Procedure GosCreateDrawingSurface({$IF defined(ANDROID)}
                                 Var aBitmap: Jbitmap;
                                 var aCanvas: Jcanvas;
                                 {$ELSEIF defined(IOS)}
                                 ...
                                 const w: integer;
                                 const h: integer);
procedure GosFreeDrawingSurface({$IF defined(ANDROID)}
                               Var aBitmap: Jbitmap;
                               var aCanvas: Jcanvas
                               {$ELSEIF defined(IOS)}
                               var aBitmapSurface: TbitmapSurface;
                               ...
                               {$ENDIF});
{$IF defined(IOS)}
Procedure GosCreateDrawingSurfaceV2(var aBitmapSurface: TbitmapSurface;
                                   Var aContext: CGContextRef;
                                   Var aColorSpace: CGColorSpaceRef;
                                   const w: integer;
                                   const h: integer);
procedure GosFreeDrawingSurfaceV2(var aBitmapSurface: TbitmapSurface;
                                 Var aContext: CGContextRef);
{$ENDIF}


{$IF defined(ANDROID)}
function GosfontStyleToAndroidStyle(const afontStyle: TfontStyles): integer;
{$ENDIF}

{$IF defined(ANDROID)}
function GosStringsToJArrayList(const AStrings: TArray<String>): JArrayList;
function GosJSetToStrings(const ASet: JSet): TArray<String>;
{$ENDIF}

{$IF defined(IOS)}
function GosStringsToNSArray(const AStrings: TArray<String>): NSMutableArray;
function GosNSSetToStrings(const ANSSet: NSSet): TArray<String>;
{$ENDIF}
...
Type
  ...
  {$ENDIF}
  TGosControlAccessPrivate = class(TFmxObject)
  ...
  {$ENDIF}
  TGosTextLayoutAccessPrivate = class abstract
  public const
...
{**********************************************}
procedure TGosShadow.Assign(Source: TPersistent);
var aSaveChange: TNotifyEvent;
begin
  if Source is TGosShadow then begin
    aSaveChange := FOnChanged;
    FOnChanged := nil;
    fenabled := TGosShadow(Source).fenabled;
    fblur := TGosShadow(Source).fblur;
    fOffsetX := TGosShadow(Source).fOffsetX;
    fOffsetY := TGosShadow(Source).fOffsetY;
    fShadowColor := TGosShadow(Source).fShadowColor;
...
{***************************************}
function TGosShadow.IsblurStored: Boolean;
begin
  result := not SameValue(fBlur, 12, Tepsilon.Position);
end;

{******************************************}
function TGosShadow.IsOffsetXStored: Boolean;
begin
  result := not SameValue(fBlur, 0, Tepsilon.Position);
end;

{******************************************}
function TGosShadow.IsOffsetYStored: Boolean;
begin
  result := not SameValue(fBlur, 0, Tepsilon.Position);
end;

{***************************************************}
procedure TGosShadow.SetEnabled(const Value: boolean);
begin
...
{***********************************************}
procedure TGosShadow.setblur(const Value: Single);
begin
...
{**************************************************}
procedure TGosShadow.setOffsetX(const Value: Single);
begin
...
{**************************************************}
procedure TGosShadow.setOffsetY(const Value: Single);
begin
...
{***********************************************************}
procedure TGosShadow.setShadowColor(const Value: TAlphaColor);
begin
...
begin
  ...
  if Assigned(GosCustomConvertFontFamilyProc) then begin
    Result := GosCustomConvertFontFamilyProc(AFamily, aFontStyles);
...
begin
  ...
  if (aControl is TGosText) then begin
    TGosText(aControl).doubleBuffered := True;
    TGosText(aControl).MakeBufBitmap;
  end
  else if (aControl is TGosRectangle) then begin
    TGosRectangle(aControl).doubleBuffered := True;
    TGosRectangle(aControl).MakeBufBitmap;
  end
  else if (aControl is TGosCircle) then begin
    TGosCircle(aControl).doubleBuffered := True;
    TGosCircle(aControl).MakeBufBitmap;
  end
  else if (aControl is TGosImage) then TGosImage(aControl).MakeBufBitmap
  else if (aControl is TGosAniIndicator) then TGosAniIndicator(aControl).MakeBufBitmap
  else if (aControl is TGosCheckBox) then TGosCheckBox(aControl).MakeBufBitmap
  else if (aControl is TGosLine) then begin
    TGosLine(aControl).doubleBuffered := True;
    TGosLine(aControl).MakeBufBitmap;
  end;

  for aChild in aControl.Controls do
    GosFmxMakeBufBitmaps(aChild);
...
{**************************************************************************************}
function  GosAlignDimensionToPixelRound(const Rect: TRectF; const Scale: single): TRectF;
begin
  result := Rect;
...
{*******************************************************************************************}
function  GosAlignDimensionToPixelRound(const Dimension: single; const Scale: single): single;
begin
  result := Round(Dimension * Scale) / Scale;
end;

{*****************************************************************}
function  GosAlignDimensionToPixelRound(const Rect: TRectF): TRectF;
begin
  result := Rect;
...
{*************************************************************************************}
function  GosAlignDimensionToPixelCeil(const Rect: TRectF; const Scale: single): TRectF;
begin
  result := Rect;
...
{******************************************************************************************}
function  GosAlignDimensionToPixelCeil(const Dimension: single; const Scale: single): single;
begin
  result := ceil(Dimension * Scale - TEpsilon.Vector) / Scale;
end;

{****************************************************************}
function  GosAlignDimensionToPixelCeil(const Rect: TRectF): TRectF;
begin
  result := Rect;
...
{********************}
{$IF defined(ANDROID)}
constructor TGosBreakTextItem.Create;
begin
  inherited;
  Line := nil;
  isEllipsis := False;
...
{*********************}
{$IF defined(ANDROID)}
destructor TGosBreakTextItem.Destroy;
begin
  line := Nil;
  inherited;
end;
...
var aBreakTextItemsStartCount: integer;
    aBreakTextItem: TGosBreakTextItem;
  ...
  begin
    if aEllipsisLine = nil then begin
      ...
      if aEllipsisFontName <> '' then begin
        ...
        aTypeface := TJTypeface.JavaClass.create(JStr1, GosfontStyleToAndroidStyle(aEllipsisFontStyle));
    ...
    try
      ...
      while ATextIdx < ATextLn do begin
       ...
       _splitLigature(aMaxWidth - aLineIndent);
        ...
        //init aBreakedText
        aBreakTextItem := TGosBreakTextItem.Create;
        try
...
begin
  result := GosBreakText(aPaint, // const aPaint: JPaint;
                        ...
                        aBreakTextItems, // const aBreakTextItems: TGosBreakTextItems;
...
var aBreakTextItemsStartCount: integer;
    aBreakTextItem: TGosBreakTextItem;
    aEllipsisBreakTextItem: TGosBreakTextItem;
...
begin
  ...
  try
    ...
    //create aFont
    aFont := GosGetCTFontRef(aFontName, aFontSize, aFontStyle); // Returns a new font reference for the given name.
    ...
    try
      ...
      try
        ...
        try
          ...
          aFramePath := CGPathCreateWithRect(GosLowerLeftCGRect(tpointf.create(0,0){aUpperLeftOrigin},
                                                               ARect.Width{aWidth},
          ...
          if (aBreakTextItems.Count > aBreakTextItemsStartCount) and  //if the text was at least breaked in one line
            ...
            if (aTrimming <> TTextTrimming.None) and
              ...
              try
                ...
                aEllipsisFont := GosGetCTFontRef(aFontName, aFontSize, aEllipsisFontStyle); // Returns a new font reference for the given name.
                                  ...
                                  try
                                    ...
                                    if aFrame <> nil then begin // CTFramesetterCreateFrame return NULL if unsuccessful.
                                      try
                                        ...
                                        aEllipsisBreakTextItem := TGosBreakTextItem.Create;
                                        try
                                          ...
                                          GosFreeAndNil(aEllipsisBreakTextItem);
                                          raise;
                                        end;
...
var aTotalLines: integer;
    aAllTextDrawed: boolean;
begin
  result := GosBreakText(aColorSpace, // const aColorSpace: CGColorSpaceRef;
...
var aBreakTextItemsStartCount: integer;
  ...
  begin
    if aEllipsisLine = '' then begin
      ...
      //-----
      GosbreakText(aFontSize, // const aFontSize: single;
...
begin
  ...
  if comparevalue(aFirstLineIndent.y + Adescent + (-1*aAscent),aMaxHeight,Tepsilon.position) <= 0 then begin
    ...
    while ATextIdx < ATextLn do begin
      ...
      //calculate the number of char in the current line (this work good also if aline is empty)
      aNumberOfChars := GosbreakText(aFontSize,
                                    aFontStyle,
                                    aFontName,
                                    aLine {text},
...
{*********************************************}
constructor TGosDrawMultiLineTextOptions.Create;
begin
  //-----
  FontName := '';
  FontSize := 12;
...
{*********************************************}
destructor TGosDrawMultiLineTextOptions.Destroy;
begin
  GosFreeAndNil(Fill);
  GosFreeAndNil(Stroke);
  inherited destroy;
end;

{*********************}
{$ZEROBASEDSTRINGS OFF} // << the guy who introduce zero base string in delphi is just a mix of a Monkey and a Donkey !
function  GosDrawMultiLineText(const aText: String; // support only those html tags :
                                                   //   <b>...</b>
                                                   //   <i>...</i>
                                                   //   <font color="#xxxxxx">...</font>
                                                   //   <span id="xxx">...</span>
                              ...
                              var aElements: TGosTextElements; // out => the list of rect describing all span elements
                              var aEllipsisRect: TRectF; // out => the rect of the Ellipsis (if present)
                              const aOptions: TGosDrawMultiLineTextOptions): {$IFDEF _USE_TEXTURE}TTexture{$ELSE}Tbitmap{$ENDIF};
...
var {$IF defined(ANDROID)}
    ...
    aImg: Tbitmap;
    {$ENDIF}
    aBreakedTextItems: TGosBreakTextItems;
    aBreakedTextItem: TGosBreakTextItem;
...
begin
  ...
  {$ENDIF}
    ...
    aBreakedTextItems := TGosBreakTextItems.Create(true{aOwnsObjects});
    try
      ...
      while P1 <= length(aText) do begin
        ...
        if aCurrText <> '' then begin
          ...
          while True do begin
            ...
            aTmpTextBreaked := GosBreakText(aPaint, // const aPaint: JPaint;
                                           ...
                                           aOptions.MaxLines - aTotalLines + GosifThen(aTotalLines > 0, 1, 0)); // const aMaxlines: integer = 0
            JStr1 := nil;
            JStr2 := nil;
            {$ELSEIF defined(IOS)}
            aTmpTextBreaked := GosBreakText(aColorSpace, // const aColorSpace: CGColorSpaceRef;
                                           aFontColor, // const aFontColor: TalphaColor;
                                           ...
                                           TTextAlign.Leading, TTextAlign.Leading, // const AHTextAlign, AVTextAlign: TTextAlign;
                                           aOptions.Trimming, // const aTrimming: TTextTrimming;
                                           aBreakedTextItems, // const aBreakTextItems: TGosBreakTextItems;
                                           aTmpTotalLines, // var aTotalLines: integer;
                                           aTmpAllTextDrawed, // var aAllTextDrawed: boolean; // out => true if all the text was drawed (no need of any Ellipsis)
                                           aFirstLineIndent, // const aFirstLineIndent: TpointF;
                                           aOptions.LineSpacing, // const aLineSpacing: single = 0;
                                           ...
                                           aOptions.MaxLines - aTotalLines + GosifThen(aTotalLines > 0, 1, 0)); // const aMaxlines: integer = 0
            {$ELSE}
            aTmpTextBreaked := GosBreakText(aFontColor, // const aFontColor: TalphaColor;
                                           aOptions.FontSize, // const aFontSize: single;
                                           aStyle, // const aFontStyle: TFontStyles;
                                           aOptions.FontName, // const aFontName: String;
                                           ATmpRect, // var ARect: TRectF;
          ...
          for I := aBreakedTextItemsCount to aBreakedTextItems.Count - 1 do begin
            ...
            else aBreakedTextItem.FontStyle := GosfontStyleToAndroidStyle(aOptions.EllipsisFontStyle);
            {$ELSE}
            if (not aBreakedTextItem.isEllipsis) then aBreakedTextItem.FontStyle := aStyle
          ...
          // else break; << we can't break here, it's maybe juste a ' ' we try to write at the end of the line that was deleted by GosBreakText
      ...
      aRect := GosAlignDimensionToPixelCeil(aRect);
      ...
      try
        ...
        for i := 0 to aBreakedTextItems.count - 1 do begin
          ...
          if aBreakedTextItem.imgSrc <> '' then begin
            ...
            aTmpRect := GosAlignToPixelRound(
                          TrectF.Create(0,0,aMaxWidth,aMaxWidth).
                            CenterAt(aBreakedTextItem.rect));
            aImg := GosLoadFitIntoResourceImageV2(aBreakedTextItem.imgSrc, aTmpRect.Width, aTmpRect.Height);
        ...
        //create the result
        result := GosJBitmaptoTexture(aBitmap);
      ...
      //create the drawing surface
      GosCreateDrawingSurfaceV2(aBitmapSurface, // var aBitmapSurface: TbitmapSurface;
      ...
      try
            ...
            if aImg <> nil then begin
              Try
                ...
                CGContextDrawImage(aContext, // c: The graphics context in which to draw the image.
                                   GosLowerLeftCGRect(TPointF.Create(aTmpRect.left, aTmpRect.top),
                                                     aTmpRect.Width,
                                                     aTmpRect.Height,
        ...
        GosFreeDrawingSurfaceV2(aBitmapSurface, aContext);
      end;
      ...
      //create the drawing surface
      GosCreateDrawingSurface(result, // Var aBitmap: Jbitmap;
      ...
      try
        ...
        try
          ...
          if (aOptions.Fill.Kind <> TbrushKind.None) or
             (aOptions.stroke.Kind <> TbrushKind.None) then begin
            GosPaintRectangle(result.Canvas, // const aBitmap: Jbitmap;
                             aRect, // const aRect: TrectF;
                             aOptions.Fill, // const Fill: TBrush;
                             aOptions.Stroke, // const Stroke: TStrokeBrush;
                             nil, // const Shadow: TGosShadow
        ...
        GosFreeDrawingSurface(result);
        raise;
      end;
      {$ENDIF}

    finally
      GosFreeAndNil(aBreakedTextItems);
      GosFreeAndNil(aFontColors);
      GosFreeAndNil(aSpanIds);
    end;
...
var aAscent: single;
    ...
    aFirstPos: TpointF;
    aLastPos: TpointF;
    aElements: TGosTextElements;
    aEllipsisRect: TRectF;
begin
  result := GosDrawMultiLineText(aText,
...
function GosGetDrawingShapeRectAndSetThickness(const Rect: TrectF;
                                              const Fill: TBrush;
                                              const Stroke: TStrokeBrush;
                                              const Fit: Boolean;
...
procedure GosPaintRectangle({$IF defined(ANDROID)}
  ...
  begin
    ...
    if ((compareValue(xRadius, 0, TEpsilon.position) = 0) or
       ...
       (sides=[TSide.Top, TSide.Left, TSide.Bottom, TSide.Right]) then begin
     //-----
     CGContextAddRect(aContext, GosLowerLeftCGRect(aRect.TopLeft,
                                                  aRect.Width,
                                                  aRect.Height,
                                                  aGridHeight));
     //-----
...
begin
  ...
  if Fill.Kind <> TBrushKind.None then begin
    ...
    if Fill.Kind = TBrushKind.Gradient then begin
      if Fill.Gradient.Style = TGradientStyle.Radial then begin
        ...
        aRadialShader := nil;
        GosFreeAndNil(aColors);
        GosFreeAndNil(aStops);
      end
  ...
  if Fill.Kind <> TBrushKind.None then begin
    ...
    if Fill.Kind = TBrushKind.Gradient then begin
      if Fill.Gradient.Style in [TGradientStyle.Radial,
        ...
        //-----
        aCallback.version := 0;
        aCallback.evaluate := @GosGradientEvaluateCallback;
              ...
              try
                ...
                try
                  ...
                  try
                    ...
                    //-----
                    CGContextDrawImage(aContext, // c: The graphics context in which to draw the image.
                                       GosLowerLeftCGRect(aRect.TopLeft,
                                                         aRect.Width,
                                                         aRect.Height,
  ...
  StrokeThicknessRestoreValue := Stroke.Thickness;
  try
    LShapeRect := GosGetDrawingShapeRectAndSetThickness(DstRect, Fill, Stroke, False, FillShape, DrawShape, StrokeThicknessRestoreValue);
...
begin
  ...
  if Fill.Kind <> TBrushKind.None then begin
      ...
      if Fill.Gradient.Style = TGradientStyle.Radial then begin
        ...
        try
          ...
          try
            ...
            CGContextAddEllipseInRect(aContext, GosLowerLeftCGRect(aRect.TopLeft,
                                                                  aRect.Width,
                                                                  aRect.Height,
                                                                  aGridHeight));
    ...
    //fill with solid color
      ...
      //-----
      CGContextFillEllipseInRect(aContext, GosLowerLeftCGRect(aRect.TopLeft,
                                                             aRect.Width,
                                                             aRect.Height,
                                                             aGridHeight));
      //-----
  ...
  if Stroke.Kind <> TBrushKind.None then begin
    ...
    if Stroke.Kind = TBrushKind.Solid then begin
      ...
      CGContextSetRGBStrokeColor(aContext, aAlphaColor.R, aAlphaColor.G, aAlphaColor.B, aAlphaColor.A);
      CGContextSetLineWidth(aContext, Stroke.Thickness);
      CGContextStrokeEllipseInRect(aContext, GosLowerLeftCGRect(aRect.TopLeft,
                                                               aRect.Width,
                                                               aRect.Height,
                                                               aGridHeight));
    end;
  ...
  StrokeThicknessRestoreValue := Stroke.Thickness;
  try
    LShapeRect := GosGetDrawingShapeRectAndSetThickness(DstRect, Fill, Stroke, True, FillShape, DrawShape, StrokeThicknessRestoreValue);
    if FillShape then
...
initialization
  GosCustomConvertFontFamilyProc := nil;
  {$IFDEF ANDROID}
  AGosiewStackCount := 0;
  _RenderScript := nil;
  {$ENDIF}
