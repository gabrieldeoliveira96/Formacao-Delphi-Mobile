...
uses System.Classes,
     ...
     uGosFmxFilterEffects;

type

  {******************************************************}
  TGosCanvasTextureMaterial = class(TCanvasTextureMaterial)
  private
  protected
  public
  end;
  ...
  {*******************************************************************}
  TGosCanvasExternalOESTextureMaterial = class(TGosCanvasTextureMaterial)
  private
  protected
  ...
  {***********************************************************************************************}
  TGosCanvasExternalOESColorAdjustEffectTextureMaterial = class(TGosCanvasExternalOESTextureMaterial)
  private
    fShaderVariables: TGosColorAdjustShaderVariables;
  protected
  ...
  public
    ...
    property ShaderVariables: TGosColorAdjustShaderVariables read fShaderVariables;
  end;

  {************************************************************************************}
  TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial = class(TGosCanvasTextureMaterial)
  private
  ...
  {*********************************************************************************************************************************}
  TGosCanvas420YpCbCr8BiPlanarVideoRangeColorAdjustEffectTextureMaterial = class(TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial)
  private
    fShaderVariables: TGosColorAdjustShaderVariables;
  protected
    procedure DoApply(const Context: TContext3D); override;
  ...
  {************************************************************************}
  TGosCanvas420YpCbCr8PlanarTextureMaterial = class(TGosCanvasTextureMaterial)
  private
  ...
  {*********************************************************************************************************}
  TGosCanvas420YpCbCr8PlanarColorAdjustEffectTextureMaterial = class(TGosCanvas420YpCbCr8PlanarTextureMaterial)
  private
    fShaderVariables: TGosColorAdjustShaderVariables;
  protected
    procedure DoApply(const Context: TContext3D); override;
  ...
  {$ENDIF}
  TGosTextureAccessPrivate = class(TInterfacedPersistent)
  public
    FWidth: Integer;
    FHeight: Integer;
  ...
  {**************************}
  TGosTexture = class(TTexture)
  private
    class var FDefExternalOESMaterial: TGosCanvasExternalOESTextureMaterial;
    class var FDef420YpCbCr8BiPlanarVideoRangeMaterial: TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial;
    class var FDef420YpCbCr8PlanarMaterial: TGosCanvas420YpCbCr8PlanarTextureMaterial;
  private
    class function getDefExternalOESMaterial: TGosCanvasExternalOESTextureMaterial; static;
    class function getDef420YpCbCr8BiPlanarVideoRangeMaterial: TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial; static;
    class function getDef420YpCbCr8PlanarMaterial: TGosCanvas420YpCbCr8PlanarTextureMaterial; static;
  private
    fMaterial: TGosCanvasTextureMaterial;
  protected
  public
    constructor Create; override;
    destructor Destroy; override;
    ...
    {$ENDIF}
    property Material: TGosCanvasTextureMaterial read fMaterial write fMaterial;
    class property DefExternalOESMaterial: TGosCanvasExternalOESTextureMaterial read getDefExternalOESMaterial;
    class property Def420YpCbCr8BiPlanarVideoRangeMaterial: TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial read getDef420YpCbCr8BiPlanarVideoRangeMaterial;
    class property Def420YpCbCr8PlanarMaterial: TGosCanvas420YpCbCr8PlanarTextureMaterial read getDef420YpCbCr8PlanarMaterial;
  end;
  ...
  TGosTextureFormat = (f420YpCbCr8BiPlanarVideoRange, f420YpCbCr8Planar);

  {************************************}
  TGosBiPlanarTexture = class(TGosTexture)
  private
    FSecondTexture: TTexture;
    FFormat: TGosTextureFormat;
  protected
  public
    constructor Create; override;
    destructor Destroy; override;
    property SecondTexture: TTexture read FSecondTexture;
    property Format: TGosTextureFormat read fformat write fformat;
  end;

  {**********************************}
  TGosPlanarTexture = class(TGosTexture)
  private
    FSecondTexture: TTexture;
  ...
  {**************}
  {$IFDEF ANDROID}
  TGosCustomAndroidContextAccess = class(TCustomAndroidContext); // << i need to access protected CreateSharedContext
  {$ELSEIF defined(IOS)}
  TGosCustomContextIOSAccess = class(TCustomContextIOS); // << i need to access protected CreateSharedContext
  {$ENDIF}

{$IF defined(ANDROID)}
procedure AGosnitializeExternalOESTexture(const Texture: TGosTexture);
{$ENDIF}
...
{****************************}
constructor TGosTexture.Create;
begin
...
{****************************}
destructor TGosTexture.Destroy;
begin
  {$IFDEF DEBUG}
  if PixelFormat <> TPixelFormat.None then AtomicDecrement(TotalMemoryUsedByTextures, Width * Height * BytesPerPixel);
  {$ENDIF}
  fMaterial := nil; // FMaterial property is not created/owned by TGosTexture
  inherited Destroy;
end;

{***************************************************************************************}
class function TGosTexture.getDefExternalOESMaterial: TGosCanvasExternalOESTextureMaterial;
var LMaterial: TGosCanvasExternalOESTextureMaterial;
begin
  ...
  if FDefExternalOESMaterial = nil then begin
    if TThread.Current.ThreadID <> MainThreadID then raise Exception.Create('TGosTexture.DefExternalOESMaterial can only be created inside the main UI thread');
    LMaterial := TGosCanvasExternalOESTextureMaterial.Create;
    if AtomicCmpExchange(Pointer(FDefExternalOESMaterial), Pointer(LMaterial), nil) <> nil then GosfreeAndNil(LMaterial)
    {$IFDEF AUTOREFCOUNT}
...
{*************************************************************************************************************************}
class function TGosTexture.getDef420YpCbCr8BiPlanarVideoRangeMaterial: TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial;
var LMaterial: TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial;
begin
  ...
  if FDef420YpCbCr8BiPlanarVideoRangeMaterial = nil then begin
    if TThread.Current.ThreadID <> MainThreadID then raise Exception.Create('TGosTexture.Def420YpCbCr8BiPlanarVideoRangeMaterial can only be created inside the main UI thread');
    LMaterial := TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial.Create;
    if AtomicCmpExchange(Pointer(FDef420YpCbCr8BiPlanarVideoRangeMaterial), Pointer(LMaterial), nil) <> nil then GosfreeAndNil(LMaterial)
    {$IFDEF AUTOREFCOUNT}
...
{*************************************************************************************************}
class function TGosTexture.getDef420YpCbCr8PlanarMaterial: TGosCanvas420YpCbCr8PlanarTextureMaterial;
var LMaterial: TGosCanvas420YpCbCr8PlanarTextureMaterial;
begin
  ...
  if FDef420YpCbCr8PlanarMaterial = nil then begin
    if TThread.Current.ThreadID <> MainThreadID then raise Exception.Create('TGosTexture.Def420YpCbCr8PlanarMaterial can only be created inside the main UI thread');
    LMaterial := TGosCanvas420YpCbCr8PlanarTextureMaterial.Create;
    if AtomicCmpExchange(Pointer(FDef420YpCbCr8PlanarMaterial), Pointer(LMaterial), nil) <> nil then GosfreeAndNil(LMaterial)
    {$IFDEF AUTOREFCOUNT}
...
// end;
//
procedure TGosTexture.Assign(Source: TPersistent);
var
  M: TBitmapData;
begin
  ...
  if Source is TBitmap then begin
    ...
    {$ENDIF}
      ...
      TGosTextureAccessPrivate(self).fTextureScale := TBitmap(Source).BitmapScale;
    ...
    {$ENDIF}
      if Handle <> 0 then TContextManager.DefaultContextClass.FinalizeTexture(Self);
      TGosTextureAccessPrivate(self).FWidth := TTexture(Source).width;
      TGosTextureAccessPrivate(self).FHeight := TTexture(Source).height;
      TGosTextureAccessPrivate(self).FPixelFormat := TTexture(Source).PixelFormat;
      TGosTextureAccessPrivate(self).FHandle := TTexture(Source).Handle;
      TGosTextureAccessPrivate(self).FStyle := TTexture(Source).Style + [TTextureStyle.Volatile];
      TGosTextureAccessPrivate(self).FMagFilter := TTexture(Source).MagFilter;
      TGosTextureAccessPrivate(self).FMinFilter := TTexture(Source).MinFilter;
      TGosTextureAccessPrivate(self).FTextureScale := TTexture(Source).TextureScale;
      //FRequireInitializeAfterLost: Boolean;
      //FBits: Pointer;
      //FContextLostId: Integer;
      //FContextResetId: Integer;
  ...
  if TThread.GetTickCount - AtomicCmpExchange(LastTotalMemoryUsedByTexturesLog, 0, 0) > 1000 then begin // every 1 sec
    ...
    GosLog('TGosTexture', 'TotalMemoryUsedByTextures: ' + ALFormatFloatU('0.##', AtomicCmpExchange(TotalMemoryUsedByTextures, 0, 0) / 1000000, ALDefaultFormatSettingsU) +' MB', TGosLogType.verbose);
  end;
  if TGosTextureAccessPrivate(self).FBits <> nil then
    GosLog('TGosTexture.Assign', 'Bits: ' + ALFormatFloatU('0.##',(Width * Height * BytesPerPixel) / 1000, ALDefaultFormatSettingsU) +' kB', TGosLogType.Warn);
  {$WARNINGS ON}
  {$ENDIF}
...
{********************}
{$IF defined(ANDROID)}
procedure TGosTexture.Assign(Source: jbitmap);
var Tex: GLuint;
begin
    ...
    if not (IsEmpty) then begin
      ...
      begin
        ...
        if (TCustomAndroidContext.GLHasAnyErrors()) then
          RaiseContextExceptionFmt(@SCannotCreateTexture, ['TGosTexture']);
      end;
    end;
  {$IF CompilerVersion >= 32} // tokyo
  //finally
...
{************************************}
constructor TGosBiPlanarTexture.Create;
begin
  inherited;
  FSecondTexture := TGosTexture.Create;
  FFormat := TGosTextureFormat.f420YpCbCr8BiPlanarVideoRange;
end;

{************************************}
destructor TGosBiPlanarTexture.Destroy;
begin
  GosfreeAndNil(FSecondTexture);
  inherited;
end;

{**********************************}
constructor TGosPlanarTexture.Create;
begin
  inherited;
  FSecondTexture := TGosTexture.Create;
  FThirdTexture := TGosTexture.Create;
  FFormat := TGosTextureFormat.f420YpCbCr8Planar;
end;

{**********************************}
destructor TGosPlanarTexture.Destroy;
begin
  GosfreeAndNil(FSecondTexture);
  GosfreeAndNil(FThirdTexture);
  inherited;
end;
...
begin
  ...
  {$IFDEF ANDROID}
  if Texture.PixelFormat = TPixelFormat.None then Texture.PixelFormat := TGosCustomAndroidContextAccess.PixelFormat;
  {$ELSEIF defined(IOS)}
  if Texture.PixelFormat = TPixelFormat.None then Texture.PixelFormat := TGosCustomContextIOSAccess.PixelFormat;
  {$ENDIF}

  {$IF CompilerVersion >= 32} // tokyo

  {$IFDEF ANDROID}
  if TGosCustomAndroidContextAccess.valid then
  {$ELSEIF defined(IOS)}
  if TGosCustomContextIOSAccess.valid then
  {$ENDIF}

  {$ELSE}

  {$IFDEF ANDROID}
  TGosCustomAndroidContextAccess.CreateSharedContext;
  if TGosCustomAndroidContextAccess.IsContextAvailable then
  {$ELSEIF defined(IOS)}
  TGosCustomContextIOSAccess.CreateSharedContext;
  if TGosCustomContextIOSAccess.IsContextAvailable then
  {$ENDIF}
  ...
  begin
    ...
    {$IFDEF ANDROID}
    if (TGosCustomAndroidContextAccess.GLHasAnyErrors()) then
      RaiseContextExceptionFmt(@SCannotCreateTexture, [TGosCustomAndroidContextAccess.ClassName]);
    {$ELSEIF defined(IOS)}
    if (TGosCustomContextIOSAccess.GLHasAnyErrors()) then
      RaiseContextExceptionFmt(@SCannotCreateTexture, [TGosCustomContextIOSAccess.ClassName]);
    {$ENDIF}
  end;
end;
{$ENDIF}
...
{$ENDIF}
procedure TGosCanvasExternalOESTextureMaterial.DoInitialize;
begin
...
{**********************************************************************}
constructor TGosCanvasExternalOESColorAdjustEffectTextureMaterial.Create;
begin
  inherited create;
  fShaderVariables := TGosColorAdjustShaderVariables.create;
end;

{**********************************************************************}
destructor TGosCanvasExternalOESColorAdjustEffectTextureMaterial.Destroy;
begin
  GosfreeAndNil(fShaderVariables);
  inherited destroy;
end;

{************************************************************************************************}
procedure TGosCanvasExternalOESColorAdjustEffectTextureMaterial.DoApply(const Context: TContext3D);
begin
...
{$IF CompilerVersion > 33} // Rio
  {$MESSAGE WARN 'Check if FMX.Materials.Canvas.TCanvasTextureMaterial.DoInitialize is still having the same implementation as in previous version and adjust the IFDEF'}
{$ENDIF}
procedure TGosCanvasExternalOESColorAdjustEffectTextureMaterial.DoInitialize;
begin
  FVertexShader := TShaderManager.RegisterShaderFromData('cnv_texture.fvs', TContextShaderKind.VertexShader, '', [
    TContextShaderSource.Create(TContextShaderArch.DX9, [
      $00, $02, $FE, $FF, $FE, $FF, $1F, $00, $43, $54, $41, $42, $1C, $00, $00, $00, $53, $00, $00, $00, $00, $02, $FE, $FF, $01, $00, $00, $00, $1C, $00, $00, $00, $00, $01, $00, $20, $4C, $00, $00, $00,
...
{*************************************************************************************}
function TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial.getCbCrTexture: TTexture;
begin
  if (Texture is TGosBiPlanarTexture) then result := TGosBiPlanarTexture(Texture).SecondTexture
  else result := nil;
end;

{************************************************************************************************}
procedure TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial.DoApply(const Context: TContext3D);
begin
  inherited DoApply(Context);
...
{$IF CompilerVersion > 33} // Rio
  {$MESSAGE WARN 'Check if FMX.Materials.Canvas.TCanvasTextureMaterial.DoInitialize is still having the same implementation as in previous version and adjust the IFDEF'}
{$ENDIF}
procedure TGosCanvas420YpCbCr8BiPlanarVideoRangeTextureMaterial.DoInitialize;
begin
  FVertexShader := TShaderManager.RegisterShaderFromData('cnv_texture.fvs', TContextShaderKind.VertexShader, '', [
    TContextShaderSource.Create(TContextShaderArch.DX9, [
      $00, $02, $FE, $FF, $FE, $FF, $1F, $00, $43, $54, $41, $42, $1C, $00, $00, $00, $53, $00, $00, $00, $00, $02, $FE, $FF, $01, $00, $00, $00, $1C, $00, $00, $00, $00, $01, $00, $20, $4C, $00, $00, $00,
...
{***************************************************************************************}
constructor TGosCanvas420YpCbCr8BiPlanarVideoRangeColorAdjustEffectTextureMaterial.Create;
begin
  inherited create;
  fShaderVariables := TGosColorAdjustShaderVariables.create;
end;
...
{***************************************************************************************}
destructor TGosCanvas420YpCbCr8BiPlanarVideoRangeColorAdjustEffectTextureMaterial.Destroy;
begin
  GosfreeAndNil(fShaderVariables);
  inherited destroy;
end;
...
{*****************************************************************************************************************}
procedure TGosCanvas420YpCbCr8BiPlanarVideoRangeColorAdjustEffectTextureMaterial.DoApply(const Context: TContext3D);
begin
  inherited DoApply(Context);
  fshaderVariables.UpdateContext(Context);
end;
...
{$IF CompilerVersion > 33} // Rio
  {$MESSAGE WARN 'Check if FMX.Materials.Canvas.TCanvasTextureMaterial.DoInitialize is still having the same implementation as in previous version and adjust the IFDEF'}
{$ENDIF}
procedure TGosCanvas420YpCbCr8BiPlanarVideoRangeColorAdjustEffectTextureMaterial.DoInitialize;
begin
  FVertexShader := TShaderManager.RegisterShaderFromData('cnv_texture.fvs', TContextShaderKind.VertexShader, '', [
    TContextShaderSource.Create(TContextShaderArch.DX9, [
      $00, $02, $FE, $FF, $FE, $FF, $1F, $00, $43, $54, $41, $42, $1C, $00, $00, $00, $53, $00, $00, $00, $00, $02, $FE, $FF, $01, $00, $00, $00, $1C, $00, $00, $00, $00, $01, $00, $20, $4C, $00, $00, $00,
...
{***********************************************************************}
function TGosCanvas420YpCbCr8PlanarTextureMaterial.getCbTexture: TTexture;
begin
  if (Texture is TGosPlanarTexture) then result := TGosPlanarTexture(Texture).SecondTexture
  else result := nil;
end;

{***********************************************************************}
function TGosCanvas420YpCbCr8PlanarTextureMaterial.getCrTexture: TTexture;
begin
  if (Texture is TGosPlanarTexture) then result := TGosPlanarTexture(Texture).ThirdTexture
  else result := nil;
end;

{************************************************************************************}
procedure TGosCanvas420YpCbCr8PlanarTextureMaterial.DoApply(const Context: TContext3D);
begin
  inherited DoApply(Context);;
...
{$IF CompilerVersion > 33} // Rio
  {$MESSAGE WARN 'Check if FMX.Materials.Canvas.TCanvasTextureMaterial.DoInitialize is still having the same implementation as in previous version and adjust the IFDEF'}
{$ENDIF}
procedure TGosCanvas420YpCbCr8PlanarTextureMaterial.DoInitialize;
begin
  FVertexShader := TShaderManager.RegisterShaderFromData('cnv_texture.fvs', TContextShaderKind.VertexShader, '', [
    TContextShaderSource.Create(TContextShaderArch.DX9, [
      $00, $02, $FE, $FF, $FE, $FF, $1F, $00, $43, $54, $41, $42, $1C, $00, $00, $00, $53, $00, $00, $00, $00, $02, $FE, $FF, $01, $00, $00, $00, $1C, $00, $00, $00, $00, $01, $00, $20, $4C, $00, $00, $00,
...
{***************************************************************************}
constructor TGosCanvas420YpCbCr8PlanarColorAdjustEffectTextureMaterial.Create;
begin
  inherited create;
  fShaderVariables := TGosColorAdjustShaderVariables.create;
end;
...
{***************************************************************************}
destructor TGosCanvas420YpCbCr8PlanarColorAdjustEffectTextureMaterial.Destroy;
begin
  GosfreeAndNil(fShaderVariables);
  inherited destroy;
end;
...
{*****************************************************************************************************}
procedure TGosCanvas420YpCbCr8PlanarColorAdjustEffectTextureMaterial.DoApply(const Context: TContext3D);
begin
  inherited DoApply(Context);
  fshaderVariables.UpdateContext(Context);
end;
...
{$IF CompilerVersion > 33} // Rio
  {$MESSAGE WARN 'Check if FMX.Materials.Canvas.TCanvasTextureMaterial.DoInitialize is still having the same implementation as in previous version and adjust the IFDEF'}
{$ENDIF}
procedure TGosCanvas420YpCbCr8PlanarColorAdjustEffectTextureMaterial.DoInitialize;
begin
  FVertexShader := TShaderManager.RegisterShaderFromData('cnv_texture.fvs', TContextShaderKind.VertexShader, '', [
    TContextShaderSource.Create(TContextShaderArch.DX9, [
      $00, $02, $FE, $FF, $FE, $FF, $1F, $00, $43, $54, $41, $42, $1C, $00, $00, $00, $53, $00, $00, $00, $00, $02, $FE, $FF, $01, $00, $00, $00, $1C, $00, $00, $00, $00, $01, $00, $20, $4C, $00, $00, $00,
...
initialization
  ...
  TGosTexture.FDefExternalOESMaterial := nil;
  TGosTexture.FDef420YpCbCr8BiPlanarVideoRangeMaterial := nil;
  TGosTexture.FDef420YpCbCr8PlanarMaterial := nil;
  {$IFDEF DEBUG}
