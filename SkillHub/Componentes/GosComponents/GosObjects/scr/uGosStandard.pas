...
uses
  uGosBase, uGosUtils, uGosAni, FMX.Effects, FMX.Text,
...
type
  TOnDrawText = procedure (Sender: TObject; Canvas: TCanvas;
    Text: uGosBase.TTextSettings; R: TRectF) of object;
...
type
  TDrawableProgress = class(TGosDrawable)
  private
    FWidth: Single;
    FValueOffset: Single;
...
type
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TImageView = class(TGosView)
  private
    FScaleType: TImageScaleType;
    FImage: TGosDrawable;
    procedure SetScaleType(const Value: TImageScaleType);
    function GetImage: TGosDrawable;
    procedure SetImage(const Value: TGosDrawable);
  protected
    procedure PaintBackground; override;
  ...
  published
    ...
    /// 可绘制图像
    /// </summary>
    property Image: TGosDrawable read GetImage write SetImage;
    /// <summary>
    /// 缩放类型
    /// </summary>
...
type
  ...
  /// 滚动视图
  /// </summary>
  TGosScrollView = class(TGosView, IViewTouch)
  private const
...
type
  ...
  private
    [weak] FScrollBox: TGosScrollView;
  ...
  public
    ...
    property ScrollBox: TGosScrollView read FScrollBox;
  end;
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TPullScrollView = class(TGosScrollView)
  private const
...
type
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TGosTextView = class(TGosScrollView, ICaption{$IF CompilerVersion > 30.0}, IAcceleratorKeyReceiver{$ENDIF})
  private
...
type
  TGosStyleView = class(TGosTextView)
  protected
    function CanRePaintBk(const View: IGosView; State: TViewState): Boolean; override;
  end;
...
type
  ...
  TGosButtonView = class(TGosStyleView)
  private
    FDefault: Boolean;
    FCancel: Boolean;
...
type
  ...
  private
    ...
    procedure SetTargetView(const Value: IGosView);
  ...
  published
    ...
    /// 目标View
    /// </summary>
    property TargetView: IGosView read FTargetView write SetTargetView;
    /// <summary>
    /// 效果样式
    /// </summary>
...
type
  ...
  TCustomMultiPathView = class(TGosView)
  private
...
{$IFDEF MSWINDOWS}
  uGosDialog,
{$ENDIF}
  uGosListView.Header, uGosListView.Footer;
...
procedure TGosTextView.ActionChange(Sender: TBasicAction; CheckDefaults: Boolean);
begin
...
procedure TGosTextView.AfterConstruction;
begin
...
procedure TGosTextView.DoAutoSize;
var
  W, H: Single;
begin
...
function TGosTextView.CanRePaintBk(const View: IGosView; State: TViewState): Boolean;
var
  Border: TViewBorder;
begin
...
function TGosTextView.CanTriggerAcceleratorKey: Boolean;
begin
  Result := ParentedVisible;
end;

procedure TGosTextView.Change;
begin
  DoChanged(FText);
end;

constructor TGosTextView.Create(AOwner: TComponent);
begin
...
function TGosTextView.CreateBackground: TGosDrawable;
begin
  Result := TDrawableBorder.Create(Self);
  Result.OnChanged := DoBackgroundChanged;
end;

procedure TGosTextView.DblClick;
begin
  inherited DblClick;
  Click;
end;

destructor TGosTextView.Destroy;
begin
  FreeAndNil(FText);
...
procedure TGosTextView.DoChanged(Sender: TObject);
begin
...
procedure TGosTextView.DoCheckedChange;
begin
  inherited;
  DoGroupSelected;
end;

procedure TGosTextView.DoDrawableChanged(Sender: TObject);
begin
  DoChanged(Sender);
end;

procedure TGosTextView.DoDrawBackground(var R: TRectF);
begin
end;

function TGosTextView.DoGetUpdateRect: TRectF;
begin
...
procedure TGosTextView.DoGroupSelected;
var
  I: Integer;
  Control: TControl;
  Item: TFmxObject;
  ...
  if (FGroupIndex <> 0) and (Checked) then begin
    if Assigned(ParentControl) then begin
      for I := 0 to ParentControl.ControlsCount - 1 do begin
        ...
        if (Control is TGosTextView) and (TGosTextView(Control).FGroupIndex = FGroupIndex) then
          TGosTextView(Control).Checked := False;
      end;
      ...
      for I := 0 to TCommonCustomForm(Parent).ChildrenCount - 1 do begin
        ...
        if (Item is TGosTextView) and (TGosTextView(Item).FGroupIndex = FGroupIndex) then
          TGosTextView(Item).Checked := False;
      end;
    end;
  end;
end;

procedure TGosTextView.DoLayoutChanged(Sender: TObject);
begin
...
procedure TGosTextView.DoLinkClick(const Text, URL: string);
begin
...
procedure TGosTextView.DoMouseLeave;
begin
...
procedure TGosTextView.DoPaintBackground(var R: TRectF);
begin
...
procedure TGosTextView.DoPaintText(var R: TRectF);
var
  SR: TRectF;
  LState: TViewState;
begin
...
procedure TGosTextView.DoRecalcSize(var AWidth, AHeight: Single);
var
  ASize: TSizeF;
...
procedure TGosTextView.DoScrollVisibleChange;
begin
  Resize;
end;

procedure TGosTextView.DoUpdateContentBounds;
var
  W, H: Single;
begin
...
function TGosTextView.GetAcceleratorChar: Char;
begin
  Result := #0;
end;

function TGosTextView.GetAcceleratorCharIndex: Integer;
begin
  Result := -1;
end;

function TGosTextView.GetAutoSize: Boolean;
begin
  Result := FText.AutoSize;
end;

function TGosTextView.GetData: TValue;
begin
  Result := Text;
end;

function TGosTextView.GetDefaultSize: TSizeF;
var
...
function TGosTextView.GetDrawable: TGosDrawableIcon;
begin
...
function TGosTextView.GetDrawableHeight: Integer;
begin
...
function TGosTextView.GetDrawableWidth: Integer;
begin
  if Assigned(FDrawable) and (FDrawable.IsEmpty = False) then
...
function TGosTextView.GetHtmlText: string;
begin
...
function TGosTextView.GetNeedSize: TSizeF;
begin
...
function TGosTextView.GetText: string;
begin
  if Assigned(FHtmlText) then
...
function TGosTextView.GetTextLength: Integer;
begin
  Result := FText.TextLength;
end;

procedure TGosTextView.ImagesChanged;
begin
  if Assigned(FDrawable) then
    FDrawable.Change;
...
procedure TGosTextView.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  if Assigned(FHtmlText) then
...
procedure TGosTextView.MouseMove(Shift: TShiftState; X, Y: Single);
begin
  inherited;
...
procedure TGosTextView.MouseUp(Button: TMouseButton; Shift: TShiftState; x,
  y: single);
begin
  inherited;
  if Assigned(FHtmlText) then
...
procedure TGosTextView.PaddingChanged;
begin
  HandleSizeChanged;
end;

procedure TGosTextView.PaintBackground;
var
  R: TRectF;
begin
...
procedure TGosTextView.SetAutoSize(const Value: Boolean);
begin
  FText.AutoSize := Value;
end;

procedure TGosTextView.SetData(const Value: TValue);
begin
...
procedure TGosTextView.SetDrawable(const Value: TGosDrawableIcon);
begin
  Drawable.Assign(Value);
end;

procedure TGosTextView.SetGravity(const Value: TLayoutGravity);
begin
  FGravity := Value;
  FText.Gravity := Value;
end;

procedure TGosTextView.SetGroupIndex(const Value: Integer);
begin
...
procedure TGosTextView.SetHtmlText(const Value: string);
begin
...
procedure TGosTextView.SetName(const Value: TComponentName);
var
  ChangeText: Boolean;
begin
...
procedure TGosTextView.SetNewScene(AScene: IScene);
begin
...
procedure TGosTextView.SetText(const Value: string);
begin
  if Assigned(FHtmlText) then
    FreeAndNil(FHtmlText);
  FText.Text := Value;
...
procedure TGosTextView.SetTextHint(const Value: string);
begin
...
procedure TGosTextView.SetTextSettings(const Value: uGosBase.TTextSettings);
begin
  FText.Assign(Value);
end;

function TGosTextView.TextStored: Boolean;
begin
  if Assigned(FHtmlText) and (FHtmlText.HtmlText <> '') then
    Result := False
  else
...
function TGosTextView.ToString: string;
begin
  Result := Format('%s ''%s''', [inherited ToString, FText.Text]);
end;

procedure TGosTextView.TriggerAcceleratorKey;
begin
  SetFocus;
end;
...
function TGosStyleView.CanRePaintBk(const View: IGosView; State: TViewState): Boolean;
begin
  Result := inherited CanRePaintBk(View, State);
end;
...
procedure TGosButtonView.AfterDialogKey(var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TGosButtonView.Click;
var
  O: TComponent;
begin
  inherited Click;
...
constructor TGosButtonView.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Clickable := True;
  CanFocus := True;
...
function TGosButtonView.CreateBackground: TGosDrawable;
begin
...
function TGosButtonView.GetDefaultSize: TSizeF;
begin
  Result := TSizeF.Create(80, 22);
end;

procedure TGosButtonView.KeyDown(var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
  inherited;
...
procedure TGosButtonView.SetScrollbar(const Value: TViewScroll);
begin
...
procedure TGosScrollView.Adjust(var ContentLayoutRect: TRectD);
begin
  if FContentBounds <> nil then
...
function TGosScrollView.AllowInitScrollbar: Boolean;
begin
  Result := not (csDesigning in ComponentState);
end;

procedure TGosScrollView.AniCalcChange(Sender: TObject);
var
...
procedure TGosScrollView.AniMouseDown(const Touch: Boolean; const X, Y: Single);
...
procedure TGosScrollView.AniMouseMove(const Touch: Boolean; const X, Y: Single);
var
  P: TPointD;
begin
...
procedure TGosScrollView.AniMouseUp(const Touch: Boolean; const X, Y: Single);
begin
...
procedure TGosScrollView.AniVScrollTo(const AOffset: Single;
...
function TGosScrollView.CanAnimation: Boolean;
begin
  Result := FCanAnimation;
end;

function TGosScrollView.CanInheritedCMGesture(const EventInfo: TGestureEventInfo): Boolean;
begin
...
procedure TGosScrollView.CMGesture(var EventInfo: TGestureEventInfo);
var
  LP: TPointF;
begin
...
procedure TGosScrollView.ContentAddObject(const AObject: TFmxObject);
begin
  if csDestroying in ComponentState then
    Exit;
...
procedure TGosScrollView.ContentBeforeRemoveObject(AObject: TFmxObject);
begin
end;

procedure TGosScrollView.ContentInsertObject(Index: Integer;
...
procedure TGosScrollView.ContentRemoveObject(const AObject: TFmxObject);
begin
  if csDestroying in ComponentState then
    Exit;
  if FDisableAlign then
...
constructor TGosScrollView.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FScrollbarWidth := 0;
  FContentBounds := nil;
  ...
  FDragScroll := False;
  FScrollSmallChangeFraction := TGosView.SmallChangeFraction;
...
function TGosScrollView.CreateAniCalculations: TScrollCalculations;
begin
...
function TGosScrollView.CreateScroll: TScrollBar;
begin
  Result := TSmallScrollBar.Create(Self);
end;

destructor TGosScrollView.Destroy;
begin
...
procedure TGosScrollView.DoInVisibleChange;
var
  V: Boolean;
begin
...
procedure TGosScrollView.DoMouseLeave;
begin
...
procedure TGosScrollView.DoRealign;
var
...
procedure TGosScrollView.DoScrollVisibleChange;
begin
...
procedure TGosScrollView.DoUpdateAniCalculations(const AAniCalculations: TScrollCalculations);
begin
  {$IFNDEF NEXTGEN}
...
procedure TGosScrollView.DoUpdateScrollingLimits(NeedUpdateScroll: Boolean; const ValueOffset: Double);
...
procedure TGosScrollView.FreeScrollbar;
begin
  AutoCapture := False;
  if Assigned(FScrollV) then begin
...
function TGosScrollView.GetAniCalculations: TScrollCalculations;
begin
  Result := FAniCalculations;
end;

function TGosScrollView.GetColorFromStyle(const ObjectName: string;
...
function TGosScrollView.GetContentBounds: TRectD;
begin
  if FContentBounds = nil then
    Result := TRectD.Empty
  else
...
function TGosScrollView.GetHScrollBar: TScrollBar;
begin
  Result := FScrollH;
end;

function TGosScrollView.GetHScrollBarValue: Double;
begin
...
function TGosScrollView.GetMaxScrollViewPos: Integer;
begin
  Result := Max(Round(FAniCalculations.MaxTarget.Point.Y), 0);
end;

function TGosScrollView.GetScrollingBehaviours: TScrollingBehaviours;
var
...
function TGosScrollView.GetScrollOffset: TPointF;
begin
  Result := PointF(0, 0);
end;

function TGosScrollView.GetScrollSmallChangeFraction: Single;
begin
  Result := FScrollSmallChangeFraction;
end;

function TGosScrollView.GetScrollValueH: Single;
var
  V: Double;
begin
...
function TGosScrollView.GetScrollValueV: Single;
var
  V: Double;
begin
...
function TGosScrollView.GetViewportPosition: TPointD;
var
  LScale: Double;
begin
...
function TGosScrollView.GetVScrollBar: TScrollBar;
begin
  Result := FScrollV;
end;

function TGosScrollView.GetVScrollBarValue: Double;
begin
...
function TGosScrollView.HasPhysicsStretchyScrolling: Boolean;
begin
  Result := HasTouchTracking and HasStretchyScrolling;
end;

function TGosScrollView.HasScrollingStretchGlow: Boolean;
begin
  Result := (FListingService <> nil) and (TListingTransitionFeature.ScrollGlow in FListingService.GetTransitionFeatures);
end;

function TGosScrollView.HasStretchyScrolling: Boolean;
begin
...
function TGosScrollView.HasTouchTracking: Boolean;
begin
...
procedure TGosScrollView.HScrollChange(Sender: TObject);
begin
...
procedure TGosScrollView.InitScrollbar;
begin
...
procedure TGosScrollView.InternalAlign;
var
...
procedure TGosScrollView.InvalidateContentSize;
begin
end;

function TGosScrollView.IsCanTouch: Boolean;
begin
  Result := HitTest and (FCanScrollV or FCanScrollH or FDragScroll);
end;

function TGosScrollView.IsOpaque: Boolean;
begin
  Result := False;
end;

function TGosScrollView.IsRunningOnDesktop: Boolean;
begin
  Result := TOSVersion.Platform in [pfWindows, pfMacOS, pfLinux];
end;

function TGosScrollView.IsStoredScrollbarWidth: Boolean;
begin
  Result := FScrollbarWidth > 0;
end;

function TGosScrollView.IsStoredScrollSmallChangeFraction: Boolean;
begin
  Result := FScrollSmallChangeFraction <> TGosView.SmallChangeFraction;
end;

function TGosScrollView.IsStoredScrollStretchGlowColor: Boolean;
begin
  Result := FScrollingStretchGlowColor <> DefaultScrollingStretchGlowColor;
end;

procedure TGosScrollView.MakScrollBar(SType: TViewScroll; var Obj: TScrollBar);
var
  O: TOrientation;
begin
...
procedure TGosScrollView.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  FMouseEvents := True;
...
procedure TGosScrollView.MouseMove(Shift: TShiftState; X, Y: Single);
begin
  FMouseEvents := True;
  inherited;
...
procedure TGosScrollView.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  FMouseEvents := True;
  inherited;
...
procedure TGosScrollView.MouseWheel(Shift: TShiftState; WheelDelta: Integer;
  var Handled: Boolean);
var
...
function TGosScrollView.NeedPaintScrollingStretchGlow: Boolean;
begin
  Result := HasScrollingStretchGlow and (Abs(FScrollStretchStrength) > 0);
end;

procedure TGosScrollView.PaintScrollingStretchGlow(const ACanvas: TCanvas;
  const AWidth, AHeight, AIntensity, AOpacity: Single);
var
...
procedure TGosScrollView.PullRefreshStart;
begin
end;

procedure TGosScrollView.RealignContent;
begin
  case FScrollbar of
    TViewScroll.None:
      begin
...
procedure TGosScrollView.Resize;
begin
  inherited Resize;
...
procedure TGosScrollView.ScrollBy(const Dx, Dy: Double);
begin
  if VScrollBar <> nil then
...
procedure TGosScrollView.ScrollStretchChanged;
begin
  if HasScrollingStretchGlow then
    Invalidate;
end;

procedure TGosScrollView.ScrollTo(const Dx, Dy: Double);
begin
  if VScrollBar <> nil then
...
procedure TGosScrollView.SetDragOneWay(const Value: Boolean);
begin
  FDragOneWay := Value;
end;

procedure TGosScrollView.SetDragScroll(const Value: Boolean);
begin
...
procedure TGosScrollView.SetHScrollBarValue(const Value: Double);
var
  V: TPointD;
begin
...
procedure TGosScrollView.SetScrollbarWidth(const Value: Single);
begin
...
procedure TGosScrollView.SetScrollSmallChangeFraction(const Value: Single);
begin
  FScrollSmallChangeFraction := Value;
end;

procedure TGosScrollView.SetShowScrollBars(const Value: Boolean);
begin
...
procedure TGosScrollView.SetViewportPosition(const Value: TPointD);
var
  LScale, X, Y: Double;
begin
  LScale := GetSceneScale;
...
procedure TGosScrollView.SetVScrollBarValue(const Value: Double);
var
  V: TPointD;
begin
  if (FAniCalculations <> nil) and Assigned(FScrollV) and (FCanScrollV{$IFNDEF NEXTGEN} or FDragScroll{$ENDIF}) then begin
...
procedure TGosScrollView.UpdateAniCalculations;
begin
...
procedure TGosScrollView.UpdateScrollBar(AScroll: TScrollBar; AScrollBar: TViewScroll; const ValueOffset: Double);
var
  LViewportPosition: TPointD;
  R: TRectF;
begin
...
procedure TGosScrollView.UpdateScrollStretchStrength(const NewValue: Single);
begin
...
procedure TGosScrollView.UpdateScrollWidth(const AScroll: TScrollBar);
begin
...
procedure TGosScrollView.ViewportPositionChange(const OldViewportPosition,
...
procedure TGosScrollView.VScrollChange(Sender: TObject);
begin
  if FScrolling or (FAniCalculations = nil) then Exit;
  FScrolling := True;
...
function TProgressView.CanRePaintBk(const View: IGosView; State: TViewState): Boolean;
begin
  Result := inherited CanRePaintBk(View, State);
...
function TImageView.CanRePaintBk(const View: IGosView; State: TViewState): Boolean;
begin
  Result := inherited CanRePaintBk(View, State);
...
begin
  FImage := TGosDrawable.Create(Self);
...
function TImageView.GetImage: TGosDrawable;
begin
...
procedure TImageView.SetImage(const Value: TGosDrawable);
begin
...
procedure TBadgeView.SetTargetView(const Value: IGosView);
begin
...
function TCustomMultiPathView.CanRePaintBk(const View: IGosView;
...
begin
  inherited;
  if AOwner is TGosScrollView then
    FScrollBox := TGosScrollView(AOwner);
  ClipChildren := True;
  FIsContentChanged := True;
  SetAcceptsControls(False);
end;
...
begin
  if ParentControl is TGosScrollView then
    Result := TGosScrollView(ParentControl).UpdateRect
  else
    Result := inherited DoGetUpdateRect;
end;
