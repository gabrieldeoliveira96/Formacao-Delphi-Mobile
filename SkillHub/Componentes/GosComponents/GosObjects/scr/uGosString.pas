...
type
  ...
  TGosTagParamsClass = class of TGosStrings;

  TALBasePrecompiledTag = Class(Tobject)
  private
    fTagString: ansiString;
  protected
    function GetTagParams: TGosStrings; virtual; abstract;
  public
    property TagString: ansiString read fTagString write fTagString;
    property TagParams: TGosStrings read GetTagParams;
  End;

  TALPrecompiledTag = Class(TALBasePrecompiledTag)
  private
    fTagParams: TGosStrings;
  protected
    function GetTagParams: TGosStrings; override;
  public
    constructor Create;
    destructor Destroy; override;
  End;
...
function ALFastTagReplace(Const SourceString, TagStart, TagEnd: AnsiString;
                          ...
                          Flags: TReplaceFlags;
                          ExtData: Pointer;
                          TagParamsClass: TGosTagParamsClass;
...
Procedure ALSplitTextAndTag(Const SourceString, TagStart, TagEnd: AnsiString;
                            SplitTextAndTagLst: TGosStrings;
                            IgnoreCase: Boolean);
{$ENDIF}
...
begin
  ...
  GosMove(Pbyte(FDataString)[FPosition], Buffer, Result * SizeOf(AnsiChar));
...
begin
  ...
  GosMove(Buffer, PAnsiChar(@FDataString[FPosition + SizeOf(AnsiChar)])^, Result * SizeOf(AnsiChar));
  Inc(FPosition, Result);
end;
...
Begin
  if CreateGUID(aGUID) <> S_OK then RaiseLastOSError;
  SetLength(Result, 16);
  GosMove(aGuid.D1, Result[0], 4); // D1: Cardinal;
  GosMove(aGuid.D2, Result[4], 2); // D2: Word;
  GosMove(aGuid.D3, Result[6], 2); // D3: Word;
  GosMove(aGuid.D4[0], Result[8], 8); // D4: array[0..7] of Byte;
End;
...
{************************************************************************************}
function TALPerlRegEx.Match(const aSubject: ansiString; aGroups: TGosStrings): Boolean;
var aOpts: Integer;
...
{****************************************************************}
procedure TALPerlRegEx.Split(Strings: TGosStrings; Limit: Integer);
var
...
{*******************************************************************************}
procedure TALPerlRegEx.SplitCapture(Strings: TGosStrings; Limit, Offset: Integer);
var
  Count: Integer;
  LUseOffset: Boolean;
  LOffset: Integer;
...
{***********************************************************************}
procedure TALPerlRegEx.SplitCapture(Strings: TGosStrings; Limit: Integer);
begin
...
var
  ...
  begin
    ...
    begin
      ...
      //begin
        { ANSI -- move directly }
        GosMove(pointer(AStr)^, ABuffer^, L);
  ...
  begin
    ...
    begin
      ...
      if Result then
        ...
        BytesToCopy := Len;
      GosMove(Item^, BufPtr^, BytesToCopy);
...
var
  ...
  var
    ...
    if Count > N then
      ...
      if BufPos > 0 then
      begin
        GosMove(Buffer[0], DynBuffer[I], BufPos * SizeOf(AnsiChar));
        Inc(I, BufPos);
      end;
      GosMove(P[0], DynBuffer[I], Count * SizeOf(AnsiChar));
      BufPos := 0;
    end
    else if Count > 0 then
    begin
      GosMove(P[0], Buffer[BufPos], Count * SizeOf(AnsiChar));
      Inc(BufPos, Count);
    end;
  end;
...
begin
  ...
  begin
    ...
    GosMove(DynBuffer[0], Sb[0], Length(DynBuffer) * SizeOf(AnsiChar));
    if BufPos > 0 then
      GosMove(Buffer[0], Sb[Length(DynBuffer)], BufPos * SizeOf(AnsiChar));
    //Result := String.Create(Sb);
    SetLength(Result, Length(Sb));
    GosMove(Sb[0], PansiChar(Result)^, Length(Sb));
  end
  else begin
    //Result := AnsiString.Create(Buffer, 0, BufPos);
    SetLength(Result, BufPos);
    GosMove(Buffer[0], PansiChar(Result)^, BufPos);
  end;
end;
...
begin
  ...
  begin
    ...
    if ALCompareText(Symbol, ALCopyStr(S, Pos + GosifThen(Low(ansiString)=0,1,0), Length(Symbol))) = 0 then
    begin
  ...
  begin     // ignore trailing text
    if AFormatSettings.ShortTimeFormat[Low(ansistring)] in ['0'..'9'] then  // stop at time digit
      ...
      until (Pos > high(S)) or
        ...
        (ALCompareText(AFormatSettings.TimeAMString, ALCopyStr(S, Pos + GosifThen(Low(ansiString)=0,1,0), Length(AFormatSettings.TimeAMString))) = 0) or
        (ALCompareText(AFormatSettings.TimePMString, ALCopyStr(S, Pos + GosifThen(Low(ansiString)=0,1,0), Length(AFormatSettings.TimePMString))) = 0);
  end;
...
begin
  ...
  try
    ...
    for I := length(Str) downto 1 do begin
      J := Lst.IndexOf(Str[I]);
      if J < 0 then raise EGosException.CreateFmt('Character (%s) not found in charset', [Str[I]]);
...
begin
  ...
  if CountOfCRLF > 0 then begin
    ...
    for I := 0 to CountOfCRLF - 1 do begin
      GosMove(pbyte(result)[CurrentPos], pbyte(TmpStr)[CurrentPos + (i * 2)], maximumLineLength); // pbyte(Result) to not jump inside uniqueString (Result is already unique thanks to previous SetLength))
      currentPos := currentPos + maximumLineLength;
      pbyte(TmpStr)[CurrentPos + (i * 2)] := 13;
      pbyte(TmpStr)[CurrentPos + (i * 2) +1] := 10;
    end;
    GosMove(pbyte(result)[CurrentPos], pbyte(TmpStr)[CurrentPos + (CountOfCRLF * 2)], ln-CurrentPos);
    result := TmpStr;
  end;
...
begin
  if _Base64Encoding = nil then begin
    ...
    if AtomicCmpExchange(Pointer(_Base64Encoding), Pointer(LEncoding), nil) <> nil then GosFreeAndNil(LEncoding)
    {$IFDEF AUTOREFCOUNT}
...
begin
  ...
  begin
    ...
    if Src = nil then Src := System.Ansistrings.StrEnd(P);
    GosMove(P^, Dest^, Src - P - 1 + EndSuffix);
  end;
end;
          ...
          if Found <> 0 then
            begin
                      ...
                      begin {Create or Expand Dynamic Buffer}
                        ...
                        if Buffer = @StaticBuffer then
                          begin {Create Dynamic Buffer}
                            ...
                            GosMove(StaticBuffer, Buffer^, SizeOf(StaticBuffer));
                          end
              ...
              repeat
                ...
                if Count > 0 then
                  begin
                    GosMove(PSrc^, PRes^, Count);
                    Inc(PRes, Count);
                  end;
                Inc(PSrc, Count + OldLen);
                GosMove(Pointer(NewPattern)^, PRes^, NewLen);
                Inc(PRes, NewLen);
                Inc(Match);
              until Match = Matches;
              Remainder := SrcLen - Start;
              if Remainder >= 0 then
                GosMove(PSrc^, PRes^, Remainder + 1);
          ...
          if Found <> 0 then
            begin {Match Found}
              ...
              if NewLen = OldLen then
                begin
                  GosMove(PSrc^, PRes^, SrcLen);
                  Inc(PRes, Found);
                  GosMove(Pointer(NewPattern)^, PRes^, NewLen);
                end
              else
                begin
                  GosMove(PSrc^, PRes^, Found);
                  Inc(PRes, Found);
                  Inc(PSrc, Found + OldLen);
                  GosMove(Pointer(NewPattern)^, PRes^, NewLen);
                  Inc(PRes, NewLen);
                  GosMove(PSrc^, PRes^, SrcLen - Found - OldLen);
                end;
            end
          else {No Matches Found}
            Result := S
...
{***********************************************************************************************}
// warning ALStrMove inverse the order of the original StrMove (to keep the same order as GosMove)
procedure ALStrMove(const Source: PAnsiChar; var Dest: PAnsiChar; Count: NativeInt);
begin
  GosMove(Source^, Dest^, Count);
end;
...
begin
  GosMove(Source^, Dest^, Count * sizeOf(Char));
end;
...
begin
  ...
  GosMove(Pbyte(aSourceString)[aStart-1], pointer(Result)^, aLength); // pointer(Result)^ to not jump inside uniqueString (aDestString is already unique thanks to previous SetLength))
end;
...
begin
  ...
  GosMove(Pbyte(aSourceString)[aStart-1], pointer(aDestString)^, aLength);  // pointer(aDestString)^ to not jump inside uniqueString (aDestString is already unique thanks to previous SetLength))
end;
...
begin
  ...
  if P1 <= 0 then begin
    if aRaiseExceptionIfNotFound then Raise EGosException.Createfmt('Can not find%s the text %s in %s', [GosIfThen(aOffset > 1, AlFormat(' after offset %d', [aOffset])), aStartStr, aSourceString])
    else begin
      result := '';
      exit;
    end;
  ...
  if P2 <= 0 then begin
    if aRaiseExceptionIfNotFound then Raise EGosException.Createfmt('Can not find%s the text %s in %s', [GosIfThen(aOffset > 1, AlFormat(' after offset %d', [aOffset])), aEndStr, aSourceString])
    else begin
      result := '';
      exit;
    end;
...
begin
  ...
  SetLength(Result,aLength); //  To guarantee that the string is unique, call the SetLength, SetString, or UniqueString procedures
  GosMove(PChar(aSourceString)[aStart-1], pointer(Result)^, aLength*SizeOf(Char)); // pointer(Result)^ to not jump inside uniqueString (aDestString is already unique thanks to previous SetLength))
end;
...
begin
  ...
  SetLength(aDestString,aLength); //  To guarantee that the string is unique, call the SetLength, SetString, or UniqueString procedures
  GosMove(PChar(aSourceString)[aStart-1], pointer(aDestString)^, aLength*SizeOf(Char));  // pointer(aDestString)^ to not jump inside uniqueString (aDestString is already unique thanks to previous SetLength))
end;
...
begin
  ...
  if P1 <= 0 then begin
    if aRaiseExceptionIfNotFound then Raise EGosExceptionU.Createfmt('Can not find%s the text %s in %s', [GosIfThenU(aOffset > 1, AlFormatU(' after offset %d', [aOffset])), aStartStr, aSourceString])
    else begin
      result := '';
      exit;
    end;
  ...
  if P2 <= 0 then begin
    if aRaiseExceptionIfNotFound then Raise EGosExceptionU.Createfmt('Can not find%s the text %s in %s', [GosIfThenU(aOffset > 1, AlFormatU(' after offset %d', [aOffset])), aEndStr, aSourceString])
    else begin
      result := '';
      exit;
    end;
...
begin
  fTagString := '';
  fTagParams := TGosStringList.Create;
  TGosStringList(fTagParams).Duplicates := dupIgnore;
end;
...
{**************************************************}
function TALPrecompiledTag.GetTagParams: TGosStrings;
begin
  result := fTagParams;
end;
...
Const ResultBuffSize: integer = 16384;
    ...
    begin
      ...
      GosMove(pbyte(aSourceString)[aStart-1], pbyte(Result)[ResultCurrentPos-1], aLength);
...
begin
  ...
  While (T1 > 0) and (T2 > T1) do begin
    ...
    If assigned(PrecompileProc) then begin
      ...
      ParamList := TGosStringList.Create;
      try
