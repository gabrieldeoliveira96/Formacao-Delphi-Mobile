...
uses
  uGosBase, uGosToast, uGosDialog, uGosAni,
  System.NetEncoding,
...
type
  ...
  TNotifyEventA = uGosAni.TNotifyEventA;
  TFrameAniType = uGosAni.TFrameAniType;
  TFrameAnimator = uGosAni.TFrameAnimator;
...
class function TFrameView.CreateFrame(Parent: TFmxObject;
  ...
  begin
    ...
    if (FDefaultStatusColor <> 0) and (TGosView.GetStatusHeight > 0) then begin
      while Parent <> nil do begin
        if (Parent is TCommonCustomForm) then begin
          TCommonCustomForm(Parent).Padding.Top :=
            TCommonCustomForm(Parent).Padding.Top + TGosView.GetStatusHeight;
          //TCommonCustomForm(Parent).Padding.Bottom :=
          //  TCommonCustomForm(Parent).Padding.Bottom + TGosView.GetNavigationBarHeight;
          Break;
        end;
...
begin
  Result := uGosBase.GetParentForm(Self);
end;
...
begin
  if FDefaultStatusColor <> Value then begin
    ...
    // 将背景颜色设为白色
    if (Value and $FF000000 > 0) and (FDefaultBackColor = 0){$IFDEF ANDROID} and (TGosView.GetStatusHeight > 0){$ENDIF} then
...
procedure TFrameView.SetStatusColor(const Value: TAlphaColor);
  ...
  var
    ...
    {$ENDIF}
  begin
    if TGosView.GetStatusHeight > 0 then begin
      F := ParentForm;
      if not Assigned(F) then
        Exit;
      F.Fill.Color := Value;
