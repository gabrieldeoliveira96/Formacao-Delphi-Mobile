...
uses
  ...
  {$IFDEF MSWINDOWS}uGosDebug, {$ENDIF}
  FMX.KeyMapping,
  FMX.VirtualKeyboard,
  {$IFDEF ANDROID}
...
type
  TGosEditDrawableBorder = class(TDrawableBorder);

type
  TGosEditViewBase = class(TGosView, ICaption)
  private
    FText: uGosBase.TTextSettings;
    FTextHint: string;
    FDrawable: TGosDrawableIcon;
    FIsChanging: Boolean;

    FOnTextChange: TNotifyEvent;
    FOnDrawViewBackgroud: TOnDrawViewBackgroud;

    function GetDrawable: TGosDrawableIcon;
    procedure SetDrawable(const Value: TGosDrawableIcon);
    procedure SetTextSettings(const Value: uGosBase.TTextSettings);
  ...
  protected
    ...
    function CreateBackground: TGosDrawable; override;
    function CanRePaintBk(const View: IGosView; State: TViewState): Boolean; override;
  ...
  public
    ...
    property TextSettings: uGosBase.TTextSettings read FText write SetTextSettings;
    property Drawable: TGosDrawableIcon read GetDrawable write SetDrawable;
...
type
  TGosEditDataModel = class(TDataModel)
  public const
  ...
  private
    ...
    FTextSettingsInfo: uGosBase.TTextSettings;
  ...
  public
    ...
    property TextSettingsInfo: uGosBase.TTextSettings read FTextSettingsInfo;
...
type
  TGosCustomEditView = class(TGosEditViewBase, ITextInput, ICaret, ITextActions, IVirtualKeyboardControl
    {$IF CompilerVersion > 30.0}, IReadOnly{$ENDIF})
  private
    FCursorFill: TBrush;
    FSelectionFill: TBrush;
    FModel: TGosEditDataModel;
    FTextHeight: Single;
    FLineHeight: Single;
    FLineTop: Single;
    FCharsBuffer: string;
  ...
  public
    ...
    property Model: TGosEditDataModel read FModel;
...
type
  ...
  TGosEditView = class(TGosCustomEditView)
  private
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
procedure TGosEditViewBase.AfterConstruction;
begin
...
function TGosEditViewBase.CanRePaintBk(const View: IGosView;
...
procedure TGosEditViewBase.Change;
begin
...
constructor TGosEditViewBase.Create(AOwner: TComponent);
var
...
begin
  ...
  FIsChanging := True;
  FText := uGosBase.TTextSettings.Create(Self);

  if csDesigning in ComponentState then begin
    FDrawable := TGosDrawableIcon.Create(Self);
...
function TGosEditViewBase.CreateBackground: TGosDrawable;
begin
  Result := TGosEditDrawableBorder.Create(Self, TViewBrushKind.Solid, $FFFFFFFF);
...
destructor TGosEditViewBase.Destroy;
begin
  FreeAndNil(FText);
...
procedure TGosEditViewBase.DoChanged(Sender: TObject);
begin
...
procedure TGosEditViewBase.DoDrawableChanged(Sender: TObject);
begin
  DoChanged(Sender);
end;

procedure TGosEditViewBase.DoLayoutChanged(Sender: TObject);
begin
  inherited DoLayoutChanged(Sender);
end;

procedure TGosEditViewBase.DoPaintBackground(var R: TRectF);
begin
...
procedure TGosEditViewBase.DoPaintText(var R: TRectF);
begin
  if FText.Text = '' then
...
function TGosEditViewBase.GetData: TValue;
begin
  Result := Text;
end;

function TGosEditViewBase.GetDefaultSize: TSizeF;
var
...
function TGosEditViewBase.GetDrawable: TGosDrawableIcon;
begin
...
function TGosEditViewBase.GetText: string;
begin
  Result := FText.Text;
end;

procedure TGosEditViewBase.ImagesChanged;
begin
...
procedure TGosEditViewBase.Loaded;
begin
  inherited Loaded;
  FText.OnChanged := DoChanged;
  Change;
...
procedure TGosEditViewBase.PaintBackground;
var
  R: TRectF;
begin
...
procedure TGosEditViewBase.SetData(const Value: TValue);
begin
...
procedure TGosEditViewBase.SetDrawable(const Value: TGosDrawableIcon);
begin
  Drawable.Assign(Value);
end;

procedure TGosEditViewBase.SetGravity(const Value: TLayoutGravity);
begin
  FGravity := Value;
  FText.Gravity := Value;
end;

procedure TGosEditViewBase.SetName(const Value: TComponentName);
var
  ChangeText: Boolean;
begin
...
procedure TGosEditViewBase.SetText(const Value: string);
begin
  FText.Text := Value;
end;

procedure TGosEditViewBase.SetTextHint(const Value: string);
begin
...
procedure TGosEditViewBase.SetTextSettings(const Value: uGosBase.TTextSettings);
begin
  FText := Value;
end;

function TGosEditViewBase.TextStored: Boolean;
begin
...
function TGosEditViewBase.ToString: string;
begin
...
procedure TGosEditDataModel.Change;
begin
  if FChanged then
  begin
...
constructor TGosEditDataModel.Create;
begin
  inherited Create;
...
destructor TGosEditDataModel.Destroy;
begin
  FreeAndNil(FCaret);
  inherited;
end;

procedure TGosEditDataModel.DoChange;
begin
  if (Owner <> nil) and Assigned(FOnChange) and not (csLoading in Owner.ComponentState) then
    FOnChange(Owner);
end;

procedure TGosEditDataModel.DoChangeTracking;
begin
  FChanged := True;
...
function TGosEditDataModel.DoFiltering(const Value: string): string;
begin
  Result := FilterText(Value, FilterChar);
end;

function TGosEditDataModel.DoTruncating(const Value: string): string;
begin
  Result := TruncateText(Value, MaxLength);
end;

function TGosEditDataModel.DoValidate(const Value: string): string;
begin
  Result := DoTruncating(DoFiltering(Value));
  if (Owner <> nil) and (Owner is TGosCustomEditView) and Assigned(FOnValidate) and not (csLoading in Owner.ComponentState) then
    FOnValidate(Owner, Result);
end;

function TGosEditDataModel.DoValidating(const Value: string): string;
begin
  Result := Value;
  if (Owner <> nil) and (Owner is TGosCustomEditView) and Assigned(FOnValidating) and not (csLoading in Owner.ComponentState) then
    FOnValidating(Owner, Result);
end;

function TGosEditDataModel.GetText: string;
begin
  Result := FTextSettingsInfo.Text;
end;

function TGosEditDataModel.HasSelection: Boolean;
begin
  Result := Abs(SelLength) > 0;
end;

function TGosEditDataModel.SelectedText: string;
begin
  if SelLength < 0 then
...
procedure TGosEditDataModel.SetCaret(const Value: TCaret);
begin
  FCaret.Assign(Value);
  SendMessage(MM_EDIT_CARETCHANGED);
end;

procedure TGosEditDataModel.SetCaretPosition(const Value: Integer);
begin
  FCaretPosition := EnsureRange(Value, 0, FTextSettingsInfo.Text.Length);
  SendMessage<Integer>(MM_EDIT_CARETPOSITION_CHANGED, Value);
end;

procedure TGosEditDataModel.SetCheckSpelling(const Value: Boolean);
begin
...
procedure TGosEditDataModel.SetFilterChar(const Value: string);
var
  OldText: string;
begin
...
procedure TGosEditDataModel.SetImeMode(const Value: TImeMode);
begin
...
procedure TGosEditDataModel.SetKeyboardType(const Value: TVirtualKeyboardType);
begin
...
procedure TGosEditDataModel.SetKillFocusByReturn(const Value: Boolean);
begin
...
procedure TGosEditDataModel.SetMaxLength(const Value: Integer);
begin
...
procedure TGosEditDataModel.SetPassword(const Value: Boolean);
begin
...
procedure TGosEditDataModel.SetReadOnly(const Value: Boolean);
begin
...
procedure TGosEditDataModel.SetReturnKeyType(const Value: TReturnKeyType);
begin
...
procedure TGosEditDataModel.SetSelLength(const Value: Integer);
begin
...
procedure TGosEditDataModel.SetSelStart(const Value: Integer);
begin
...
procedure TGosEditDataModel.SetText(const Value: string);
var
  OldText: string;
begin
  if Text <> Value then
...
procedure TGosEditDataModel.SetTextWithoutValidation(const Value: string);
begin
  if Text <> Value then
  begin
...
procedure TGosEditDataModel.SetTyping(const Value: Boolean);
begin
...
procedure TGosEditDataModel.ResultTextSettingsChanged;
begin
...
procedure TGosCustomEditView.BeginSelection;
begin
  SelectionMode := TSelectionMode.TextSelection;
  FTextService.BeginSelection;
end;

function TGosCustomEditView.CheckGravity(const SrcX, EditRectWidth, WholeTextWidth: Single): Single;
begin
  case FGravity of
    // 中间
...
procedure TGosCustomEditView.CMGesture(var EventInfo: TGestureEventInfo);
var
  LocalPoint: TPointF;
begin
...
procedure TGosCustomEditView.CopyToClipboard;
begin
...
constructor TGosCustomEditView.Create(AOwner: TComponent);
var
  PlatformTextService: IFMXTextService;
begin
  inherited Create(AOwner);
  EnableExecuteAction := False;
  FModel := TGosEditDataModel.Create(Self);
...
function TGosCustomEditView.CreatePopupMenu: TPopupMenu;
var
  TmpItem: TMenuItem;
begin
...
procedure TGosCustomEditView.CutToClipboard;
begin
...
procedure TGosCustomEditView.DblTap;
begin
  SelectWord;
end;

procedure TGosCustomEditView.DeleteSelection;
begin
...
destructor TGosCustomEditView.Destroy;
begin
  FModel.Receiver := nil;
  FModel.Free;
  FLoupeService := nil;
...
procedure TGosCustomEditView.DoChangeTracking;
begin
  UpdateSpelling;
end;

procedure TGosCustomEditView.DoCopy(Sender: TObject);
begin
  CopyToClipboard;
end;

procedure TGosCustomEditView.DoCut(Sender: TObject);
begin
  CutToClipboard;
end;

procedure TGosCustomEditView.DoDelete(Sender: TObject);
begin
  if Observers.IsObserving(TObserverMapping.EditLinkID) then
    if not TLinkObservers.EditLinkEdit(Observers) then
    begin
...
procedure TGosCustomEditView.DoEnter;
var
...
procedure TGosCustomEditView.DoExit;
begin
...
procedure TGosCustomEditView.DoInitStyle;
var
...
procedure TGosCustomEditView.DoLeftSelPtChangePosition(Sender: TObject; var X,
  Y: Single);
var
...
procedure TGosCustomEditView.DoLeftSelPtMouseDown(Sender: TObject;
...
procedure TGosCustomEditView.DoPaintText(var ARect: TRectF);
var
  i: Integer;
...
procedure TGosCustomEditView.DoPaste(Sender: TObject);
begin
  PasteFromClipboard;
end;

procedure TGosCustomEditView.DoRealign;
begin
  inherited DoRealign;
  RealignContent;
...
procedure TGosCustomEditView.DoRightSelPtChangePosition(Sender: TObject; var X,
  Y: Single);
var
  CurrentPoint: TPointF;
  NewSelEnd: Integer;
...
procedure TGosCustomEditView.DoRightSelPtMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  BeginSelection;
  SetLoupePosition(TSelectionPointType.Right);
...
procedure TGosCustomEditView.DoSelectAll(Sender: TObject);
begin
  SelectAll;
end;

procedure TGosCustomEditView.DoSelPtMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
  EndSelection;
end;

procedure TGosCustomEditView.DoTextChange(Sender: TObject);
begin
  {$IFDEF ANDROID}
...
procedure TGosCustomEditView.DoTyping;
begin
  if Assigned(Model.OnTyping) then
    Model.OnTyping(Self);
end;

procedure TGosCustomEditView.EndIMEInput;
{$IFDEF ANDROID}
var
  LText: string;
{$ENDIF}
...
procedure TGosCustomEditView.EndSelection;
begin
  HideLoupe;
...
function TGosCustomEditView.FindContextMenuItem(
...
function TGosCustomEditView.GetCaret: TCustomCaret;
begin
  Result := Model.Caret;
end;

function TGosCustomEditView.GetCaretPosition: Integer;
begin
...
function TGosCustomEditView.GetCharX(a: Integer): Single;
var
  WholeTextWidth: Single;
  EditRectWidth: Single;
  Rgn: TRegion;
...
function TGosCustomEditView.GetCheckSpelling: Boolean;
begin
  Result := Model.CheckSpelling;
end;

function TGosCustomEditView.GetCoordinatePosition(const Value: Single): Integer;
var
...
function TGosCustomEditView.GetEditPopupMenu: TPopupMenu;
begin
  if FEditPopupMenu = nil then
...
function TGosCustomEditView.GetFilterChar: string;
begin
  Result := Model.FilterChar;
end;

function TGosCustomEditView.GetImeMode: TImeMode;
begin
  Result := Model.ImeMode;
end;

function TGosCustomEditView.GetInputSupport: Boolean;
begin
  Result := Model.InputSupport;
end;

function TGosCustomEditView.GetKeyboardType: TVirtualKeyboardType;
begin
  Result := Model.KeyboardType;
end;

function TGosCustomEditView.GetKillFocusByReturn: Boolean;
begin
  Result := Model.KillFocusByReturn;
end;

function TGosCustomEditView.GetLength: Integer;
begin
  Result := Model.FTextSettingsInfo.Text.Length;
end;

function TGosCustomEditView.GetMaxLength: Integer;
begin
  Result := Model.MaxLength;
end;

function TGosCustomEditView.GetOnChange: TNotifyEvent;
begin
  Result := FOnModelChange;
end;

function TGosCustomEditView.GetOnChangeTracking: TNotifyEvent;
begin
  Result := Model.OnChangeTracking;
end;

function TGosCustomEditView.GetOnTyping: TNotifyEvent;
begin
  Result := Model.OnTyping;
end;

function TGosCustomEditView.GetOnValidate: TValidateTextEvent;
begin
  Result := Model.OnValidate;
end;

function TGosCustomEditView.GetOnValidating: TValidateTextEvent;
begin
  Result := Model.OnValidating;
end;

function TGosCustomEditView.GetOriginCaretPosition: Integer;
begin
  if FTextService <> nil then
...
function TGosCustomEditView.GetPassword: Boolean;
begin
  Result := Model.Password;
end;

function TGosCustomEditView.GetPasswordCharWidth: Single;
begin
  Result := FText.Font.Size / 2;
end;

function TGosCustomEditView.GetReadOnly: Boolean;
begin
  Result := Model.ReadOnly;
end;

function TGosCustomEditView.GetReturnKeyType: TReturnKeyType;
begin
  Result := Model.ReturnKeyType;
end;

function TGosCustomEditView.GetSelectionBounds: TRect;
begin
  Result := Rect(Model.SelStart, 0, Model.SelStart + Model.SelLength, 0);
end;

{$IF CompilerVersion >= 33}
function TGosCustomEditView.GetSelectionPointSize: TSizeF;
begin
...
function TGosCustomEditView.GetSelfCaret: TCaret;
begin
  Result := Model.Caret;
end;

function TGosCustomEditView.GetSelLength: Integer;
begin
  Result := Abs(Model.SelLength);
end;

{$IFDEF ANDROID}
procedure TGosCustomEditView.UpdateAndroidKeyboardServiceState;
var
...
function TGosCustomEditView.GetSelRect: TRectF;
var
...
function TGosCustomEditView.GetSelStart: Integer;
begin
...
function TGosCustomEditView.GetSelText: string;
begin
  Result := Model.SelectedText;
end;

function TGosCustomEditView.GetTargetClausePointF: TPointF;
var
  Str: String;
begin
...
function TGosCustomEditView.GetText: string;
begin
  Result := FTextService.CombinedText;
end;

function TGosCustomEditView.GetTextService: TTextService;
begin
  Result := FTextService;
end;

procedure TGosCustomEditView.GoToTextBegin;
begin
  CaretPosition := 0;
end;

procedure TGosCustomEditView.GoToTextEnd;
begin
  CaretPosition := Text.Length;
end;

function TGosCustomEditView.HasSelection: Boolean;
begin
  Result := Model.HasSelection;
end;

function TGosCustomEditView.HasText: Boolean;
begin
  Result := Text.Length > 0;
end;

function TGosCustomEditView.HaveSelectionPickers: Boolean;
begin
  Result := (FLeftSelPt <> nil) and (FRightSelPt <> nil);
end;

procedure TGosCustomEditView.HideCaret;
begin
  Model.Caret.Hide;
end;

procedure TGosCustomEditView.HideInputMethod;
{$IFDEF NEXTGEN}
var
...
procedure TGosCustomEditView.HideLoupe;
begin
  if FLoupeService <> nil then
    FLoupeService.Hide;
end;

procedure TGosCustomEditView.IMEStateUpdated;
var
  CombinedText: string;
begin
  CombinedText := FTextService.CombinedText;
...
procedure TGosCustomEditView.InsertText(const AText: string);
var
  OldText: string;
...
procedure TGosCustomEditView.KeyDown(var Key: Word; var KeyChar: System.WideChar;
  Shift: TShiftState);
var
  TmpS: string;
...
procedure TGosCustomEditView.Loaded;
begin
  inherited Loaded;
  DoInitStyle;
end;

procedure TGosCustomEditView.LongTap(const X, Y: Single);
begin
...
procedure TGosCustomEditView.MMCaretPositionChanged(
  var Message: TDispatchMessageWithValue<Integer>);
begin
  SetCaretPosition(Message.Value);
end;

procedure TGosCustomEditView.MMCheckSpellingChanged(
...
procedure TGosCustomEditView.MMEditButtonsChanged(var Message: TDispatchMessage);
begin
end;

{$IF CompilerVersion >= 32}
procedure TGosCustomEditView.MMFilterCharChanged(var Message: TDispatchMessage);
begin
  if FTextService <> nil then
...
procedure TGosCustomEditView.MMImeModeChanged(var AMessage: TDispatchMessage);
begin
  if Model.Password then
...
procedure TGosCustomEditView.MMMaxLengthChanged(var Message: TDispatchMessage);
begin
  if FTextService <> nil then
    FTextService.MaxLength := Model.MaxLength;
end;

procedure TGosCustomEditView.MMPasswordChanged(var AMessage: TDispatchMessage);
begin
  if Model.Password then
    FTextService.SetImeMode(TImeMode.imDisable)
  else
...
procedure TGosCustomEditView.MMPromptTextChanged(var Message: TDispatchMessage);
begin
  Repaint;
end;

procedure TGosCustomEditView.MMSelLenghtChanged(
...
procedure TGosCustomEditView.MMSelStartChanged(
  var AMessage: TDispatchMessageWithValue<Integer>);
begin
  UpdateSelectionPointPositions;
  RepaintEdit;
...
procedure TGosCustomEditView.MMTextChanged(var AMessage: TDispatchMessageWithValue<string>);
var
  LText: string;
begin
...
procedure TGosCustomEditView.MMTextChanging(
  var AMessage: TDispatchMessageWithValue<string>);
begin
  DoChangeTracking;
end;

procedure TGosCustomEditView.MMTextSettingsChanged(var AMessage: TDispatchMessage);
begin
...
procedure TGosCustomEditView.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
var
  NewPosition: Integer;
begin
...
procedure TGosCustomEditView.MouseMove(Shift: TShiftState; X, Y: Single);
...
procedure TGosCustomEditView.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  inherited;
  HideLoupe;
...
function TGosCustomEditView.ParentFrame: TFrame;
var
  P: TFmxObject;
begin
  Result := nil;
...
procedure TGosCustomEditView.PasteFromClipboard;
var
...
procedure TGosCustomEditView.PlayClickEffect;
begin
end;

procedure TGosCustomEditView.PMGetTextContentRect(
  var Message: TDispatchMessageWithValue<TRectF>);
begin
  Message.Value := ContentRect;
end;

procedure TGosCustomEditView.PMInit(var Message: TDispatchMessage);
begin
  if FTextService <> nil then
  begin
    FTextService.MaxLength := Model.MaxLength;
...
procedure TGosCustomEditView.RealignContent;
var
  Size: TSizeF;
  Pos: TPointF;
...
procedure TGosCustomEditView.RepaintEdit;
begin
  Repaint;
end;

procedure TGosCustomEditView.Replace(const AStartPos, ALength: Integer; const AStr: string);
begin
end;

procedure TGosCustomEditView.ResetSelection;
var
  LCaretPosition: Integer;
begin
...
procedure TGosCustomEditView.Resize;
begin
  inherited Resize;
  RealignContent;
  UpdateSpelling;
...
procedure TGosCustomEditView.SelectAll;
var
  CaretPos: Integer;
begin
...
procedure TGosCustomEditView.SelectWord;
var
...
procedure TGosCustomEditView.SetCaret(const Value: TCaret);
begin
  Model.Caret := Value;
end;

procedure TGosCustomEditView.SetCaretPosition(const Value: Integer);
var
  P: TPoint;
begin
  if FTextService <> nil then
...
procedure TGosCustomEditView.SetCheckSpelling(const Value: Boolean);
begin
  Model.CheckSpelling := Value;
end;

procedure TGosCustomEditView.SetFilterChar(const Value: string);
begin
  Model.FilterChar := Value;
end;

procedure TGosCustomEditView.SetImeMode(const Value: TImeMode);
begin
  Model.ImeMode := Value;
end;

procedure TGosCustomEditView.SetInputSupport(const Value: Boolean);
begin
  Model.InputSupport := Value;
end;

procedure TGosCustomEditView.SetKeyboardType(Value: TVirtualkeyboardType);
begin
  Model.KeyboardType := Value;
end;

procedure TGosCustomEditView.SetKillFocusByReturn(const Value: Boolean);
begin
  Model.KillFocusByReturn := Value;
end;

procedure TGosCustomEditView.SetLoupePosition(const X, Y: Single);
var
  LoupePos: TPointF;
  ZoomPos: TPointF;
begin
...
procedure TGosCustomEditView.SetMaxLength(const Value: Integer);
begin
  Model.MaxLength := Value;
end;

procedure TGosCustomEditView.SetOnChange(const Value: TNotifyEvent);
begin
  FOnModelChange := Value;
end;

procedure TGosCustomEditView.SetOnChangeTracking(const Value: TNotifyEvent);
begin
  Model.OnChangeTracking := Value;
end;

procedure TGosCustomEditView.SetOnTyping(const Value: TNotifyEvent);
begin
  Model.OnTyping := Value;
end;

procedure TGosCustomEditView.SetOnValidate(const Value: TValidateTextEvent);
begin
  Model.OnValidate := Value;
end;

procedure TGosCustomEditView.SetOnValidating(const Value: TValidateTextEvent);
begin
  Model.OnValidating := Value;
end;

procedure TGosCustomEditView.SetPassword(const Value: Boolean);
begin
  Model.Password := Value;
end;

procedure TGosCustomEditView.SetReadOnly(const Value: Boolean);
begin
  Model.ReadOnly := Value;
end;

procedure TGosCustomEditView.SetReturnKeyType(Value: TReturnKeyType);
begin
  Model.ReturnKeyType := Value;
end;

procedure TGosCustomEditView.SetLoupePosition(
...
procedure TGosCustomEditView.SetSelectionFill(const Value: TBrush);
begin
  FSelectionFill.Assign(Value);
end;

procedure TGosCustomEditView.SetSelectionMode(const Value: TSelectionMode);
begin
...
procedure TGosCustomEditView.SetSelLength(const Value: Integer);
begin
  Model.SelLength := Value;
end;

procedure TGosCustomEditView.SetSelStart(const Value: Integer);
begin
  Model.SelLength := 0;
  Model.SelStart := Value;
...
procedure TGosCustomEditView.SetText(const Value: string);
begin
...
procedure TGosCustomEditView.SetTextInternal(const Value: string);
begin
{$IF CompilerVersion > 32}
  {$IFDEF ANDROID}
...
procedure TGosCustomEditView.ShowCaret;
begin
  Model.Caret.Show;
end;

function TGosCustomEditView.ShowContextMenu(
...
procedure TGosCustomEditView.ShowLoupe;
begin
  if FLoupeService <> nil then
  begin
...
procedure TGosCustomEditView.SpellFixContextMenuHandler(Sender: TObject);
var
  LPos: Integer;
  BP, EP: Integer;
begin
...
procedure TGosCustomEditView.StartIMEInput;
begin
  FTextService.CaretPosition := Point(GetOriginCaretPosition, 0);
end;

function TGosCustomEditView.TextWidth(const AStart, ALength: Integer): Single;
var
  Rgn: TRegion;
  S, L, I: Integer;
begin
...
procedure TGosCustomEditView.UpdateCaretPosition;
var
  CaretHeight: Single;
  Pos: TPointF;
begin
...
procedure TGosCustomEditView.UpdateFirstVisibleChar;
var
...
procedure TGosCustomEditView.UpdateLayoutSize;
var
  LSize: TPointF;
begin
...
procedure TGosCustomEditView.UpdatePopupMenuItems;
var
...
procedure TGosCustomEditView.UpdateSelectionPointPositions;
var
  R: TRectF;
...
procedure TGosCustomEditView.UpdateSpelling;
begin
...
procedure TGosCustomEditView.UpdateSpellPopupMenu(const APoint: TPointF);
var
  I, J, BP, EP: Integer;
  LPos: Integer;
...
procedure TGosCustomEditView.UpdateTextHeight;
begin
  FTextHeight := 0;
...
procedure TGosCustomEditView.UpdateTextLayout;
begin
...
constructor TGosEditView.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TGosEditView.Destroy;
begin
  inherited Destroy;
end;
