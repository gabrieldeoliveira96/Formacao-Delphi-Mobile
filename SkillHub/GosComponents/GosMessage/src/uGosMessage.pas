...
uses
  ...
  FMX.Objects, FMX.StdCtrls, FMX.Ani, System.UITypes, FMX.Graphics,
  FMX.VirtualKeyboard, FMX.Platform;

type
  TIconDialog = (Success, Warning, Error, Info, Question);
  [ComponentPlatforms($FFFF)]
  TGosMessage = class(TLayout)
  private
    ...
    FColorGreen: TAlphaColor;
    FColorRed : TAlphaColor;
    FColorGray : TAlphaColor;
    FColorBlue : TAlphaColor;
    FColorOrange : TAlphaColor;
  ...
  published
    ...
    property ColorGreen : TAlphaColor read FColorGreen write FColorGreen;
    property ColorRed : TAlphaColor read FColorRed write FColorRed;
    property ColorGray : TAlphaColor read FColorGray write FColorGray;
    property ColorBlue : TAlphaColor read FColorBlue write FColorBlue;
    property ColorOrange : TAlphaColor read FColorOrange write FColorOrange;
  end;
...
procedure Register;
begin
  RegisterComponents('GosComponent', [TGosMessage]);
end;

{ TGosMessage }

constructor TGosMessage.Create(AOwner: TComponent);
begin
  inherited;
  ColorGreen:= $FF35BD27;
  ColorRed:= $FFE64D4D;
  ...
  RectMsg := TRectangle.Create(self);
  RectMsg.Opacity := 1;
  ...
  AnimacaoArco := TFloatAnimation.Create(self);
  ...
  // Retangulo botao 1...
  RectBtn1 := TRectangle.Create(self);
  RectBtn1.Opacity := 1;
  ...
  // Retangulo botao 2...
  RectBtn2 := TRectangle.Create(self);
  with RectBtn2 do
  begin
    Opacity := 1;
...
procedure TGosMessage.EndAnimationArco(Sender: TObject);
begin
...
procedure TGosMessage.EndAnimation(Sender: TObject);
begin
  Fundo.Visible := false;
  Layout.Visible := false;
...
procedure TGosMessage.ClickBtn1(Sender: TObject);
begin
  RectMsg.Tag := 1;
  CloseDialog(Sender);
end;
...
procedure TGosMessage.ClickBtn2(Sender: TObject);
begin
  RectMsg.Tag := 2;
  CloseDialog(Sender);
end;
...
procedure TGosMessage.BtnMouseDown(Sender: TObject;
...
// Trata efeito de clique nos botaoes 1 e 2...
procedure TGosMessage.BtnMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
...
// Fecha mensagem...
procedure TGosMessage.CloseDialog(Sender: TObject);
begin
  // AnimacaoFundo...
...
procedure TGosMessage.Show(icon : TIconDialog;
                           titulo, mensagem : string;
                           TextoBtn1 : string = 'OK';
                           CallBackBtn1 : TProc = nil;
                           TextoBtn2 : string = '';
...
begin
  ...
  begin
    if LabelMensagem.Height > self.Height then
      RectMsg.Height:= self.Height-20
    else
