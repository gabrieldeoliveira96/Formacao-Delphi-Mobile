...
Type
  ...
  TGosColorAdjustShaderVariables = class(TObject)
  private
    fContrast: Single;
    fHighlights: Single;
    fShadows: Single;
  ...
  {$ENDIF}
  TGosFilterBaseFilterAccessPrivate = class(TFmxObject)
  protected
    FFilter: TFilter; // << need to access this private field
    FInputFilter: TFilterBaseFilter;
  end;

  TGosFilterBaseFilter = class(TFilterBaseFilter)
  protected
    function filter: TFilter;
  end;

  [ComponentPlatforms($FFFF)]
  TGosColorAdjustEffect = class(TImageFXEffect)
  private
...
{***********************************************}
constructor TGosColorAdjustShaderVariables.Create;
begin
...
{*******************************************************************************}
procedure TGosColorAdjustShaderVariables.UpdateContext(const Context: TContext3D);
begin
...
begin
  Result := TFilterRec.Create('ALColorAdjust', 'Color adjustments operations.', [
    TFilterValueRec.Create('Contrast',    'The contrast multiplier.', TGosColorAdjustShaderVariables.defContrast,    -2,  2),
    TFilterValueRec.Create('Highlights',  'The highlights offset.',   TGosColorAdjustShaderVariables.defHighlights,   0,  1),  // 0,   0,  1 (bestminr) | 1,   0,  1 (BradLarson)
    TFilterValueRec.Create('Shadows',     'The shadows offset.',      TGosColorAdjustShaderVariables.defShadows,      0,  1),
    TFilterValueRec.Create('Whites',      'The whites offset.',       TGosColorAdjustShaderVariables.defWhites,      -1,  1),
    TFilterValueRec.Create('Blacks',      'The blacks offset.',       TGosColorAdjustShaderVariables.defBlacks,      -1,  1),
    TFilterValueRec.Create('Exposure',    'The exposure offset.',     TGosColorAdjustShaderVariables.defExposure,    -1,  1),
    TFilterValueRec.Create('Gamma',       'The gamma offset.',        TGosColorAdjustShaderVariables.defGamma,       -1,  1),
    TFilterValueRec.Create('Saturation',  'The saturation offset.',   TGosColorAdjustShaderVariables.defSaturation,  -1,  1),
    TFilterValueRec.Create('Vibrance',    'The vibrance offset.',     TGosColorAdjustShaderVariables.defVibrance,    -1,  1),
    TFilterValueRec.Create('Temperature', 'The temperature offset.',  TGosColorAdjustShaderVariables.defTemperature, -1,  1),
    TFilterValueRec.Create('Tint',        'The tint offset.',         TGosColorAdjustShaderVariables.defTint,        -1,  1)]
  );
end;

{*******************************************}
function TGosFilterBaseFilter.filter: TFilter;
begin
  result := TGosFilterBaseFilterAccessPrivate(self).FFilter;
end;

{*********************************************************}
procedure TGosColorAdjustEffect.AssignTo(Dest: TPersistent);
begin
  if Dest is TGosColorAdjustEffect then
    with TGosColorAdjustEffect(Dest) do begin
...
{************************************************}
function TGosColorAdjustEffect.GetContrast: Single;
begin
...
{**************************************************}
function TGosColorAdjustEffect.GetHighlights: Single;
begin
  if Assigned(Filter) then
...
{***********************************************}
function TGosColorAdjustEffect.GetShadows: Single;
begin
  if Assigned(Filter) then
...
{**************************************************}
function TGosColorAdjustEffect.GetSaturation: Single;
begin
  if Assigned(Filter) then
...
{************************************************}
function TGosColorAdjustEffect.GetVibrance: Single;
begin
  if Assigned(Filter) then
...
{**********************************************}
function TGosColorAdjustEffect.GetWhites: Single;
begin
  if Assigned(Filter) then
...
{**********************************************}
function TGosColorAdjustEffect.GetBlacks: Single;
begin
  if Assigned(Filter) then
...
{***************************************************}
function TGosColorAdjustEffect.GetTemperature: Single;
begin
  if Assigned(Filter) then
...
{********************************************}
function TGosColorAdjustEffect.GetTint: Single;
begin
  if Assigned(Filter) then
...
{************************************************}
function TGosColorAdjustEffect.GetExposure: Single;
begin
  if Assigned(Filter) then
...
{*********************************************}
function TGosColorAdjustEffect.GetGamma: Single;
begin
  if Assigned(Filter) then
...
{*********************************************************}
procedure TGosColorAdjustEffect.SetContrast(AValue: Single);
begin
...
{***********************************************************}
procedure TGosColorAdjustEffect.SetHighlights(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{********************************************************}
procedure TGosColorAdjustEffect.SetShadows(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{***********************************************************}
procedure TGosColorAdjustEffect.SetSaturation(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{*********************************************************}
procedure TGosColorAdjustEffect.SetVibrance(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{*******************************************************}
procedure TGosColorAdjustEffect.SetWhites(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{*******************************************************}
procedure TGosColorAdjustEffect.SetBlacks(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{************************************************************}
procedure TGosColorAdjustEffect.SetTemperature(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{*****************************************************}
procedure TGosColorAdjustEffect.SetTint(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{*********************************************************}
procedure TGosColorAdjustEffect.SetExposure(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
{******************************************************}
procedure TGosColorAdjustEffect.SetGamma(AValue: Single);
begin
  if not Assigned(Filter) then Exit;
...
procedure Register;
begin
  RegisterComponents('GosComponent', [TGosColorAdjustEffect]);
  RegisterNoIcon([TGosColorAdjustEffect]);
end;

initialization
  RegisterClasses([TGosColorAdjustEffect]);
