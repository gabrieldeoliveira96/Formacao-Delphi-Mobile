...
type
  ...
  TGosScrollingAcquiredMessage = class(TMessage)
  private
    FAcquired: boolean;
  public
...
const
  GosDefaultStorageTime = 0.15;
  GosDefaultIntervalOfAni = 10; // << on ios this is not use because we synch via cadisplaylink. however on android
  ...
  GosDecelerationRateNormal = 1.95;
  GosDecelerationRateFast = 9.5;
  GosDefaultElasticity = 100;
  GosDefaultMinVelocity = 10;
  GosDefaultMaxVelocity = 5000;
  GosDefaultDeadZone = 8;
  GosDefaultVelocityFactor = 1;

type

  TGosAniCalculations = class(TPersistent)
  public type
  ...
  private type
    ...
    private
      [Weak] fAniCalculations: TGosAniCalculations;
    protected
      function GetObjectiveCClass: PTypeInfo; override;
    public
      constructor Create(const aAniCalculations: TGosAniCalculations);
      procedure displayLinkUpdated; cdecl;
    end;
  ...
  private
    ...
    FInterval: Word;
    FCurrentVelocity: TGosPointD;
    FUpVelocity: TGosPointD;
    FUpPosition: TGosPointD;
    ...
    FDown: Boolean;
    FAnimation: Boolean;
    FViewportPosition: TGosPointD;
    FLowChanged: Boolean;
    FLastTimeChanged: TDateTime;
    FDownPoint: TGosPointD;
    FDownPosition: TGosPointD;
    ...
    function GetPositions(const Index: Integer): TGosPointD;
    function GetPositionCount: Integer;
    function GetPositionTimes(const Index: Integer): TDateTime;
    function PosToView(const APosition: TGosPointD): TGosPointD;
    procedure SetViewportPosition(const Value: TGosPointD);
  ...
  protected
    function IsSmall(const P: TGosPointD; const Epsilon: Double): Boolean; overload;
    function IsSmall(const P: TGosPointD): Boolean; overload;
    function GetOwner: TPersistent; override;
    procedure DoStart; virtual;
    procedure DoChanged; virtual;
    procedure DoStop; virtual;
    procedure DoCalc(const DeltaTime: Double; var NewPoint, NewVelocity: TGosPointD; var Done: Boolean); virtual;
    ...
    property Positions[const index: Integer]: TGosPointD read GetPositions;
    ...
    property UpVelocity: TGosPointD read FUpVelocity;
    property UpPosition: TGosPointD read FUpPosition;
    property UpDownTime: TDateTime read FUpDownTime;
    property Target: TTarget read FTarget;
    property MinVelocity: Integer read FMinVelocity write FMinVelocity default GosDefaultMinVelocity;
    property MaxVelocity: Integer read FMaxVelocity write FMaxVelocity default GosDefaultMaxVelocity;
  ...
  public
    ...
    property CurrentVelocity: TGosPointD read FCurrentVelocity write FCurrentVelocity;
    property ViewportPosition: TGosPointD read FViewportPosition write SetViewportPosition;
    property ViewportPositionF: TPointF read GetViewportPositionF write SetViewportPositionF;
    property DownPosition: TGosPointD read fDownPosition write fDownPosition;
    ...
    property DeadZone: Integer read FDeadZone write FDeadZone default GosDefaultDeadZone;
  ...
  published
    property Interval: Word read FInterval write SetInterval default GosDefaultIntervalOfAni;
...
var
  GosAniCalcTimerProcs: Tlist<TGosAniCalculations>;
...
const
  GosEpsilonPoint = TEpsilon.Position;
  GosDefaultOpacityTime = 0.3;
  GosMaxOpacity = 1.5;
  GosEpsilonTime = 0; // 0.01; => https://quality.embarcadero.com/browse/RSP-18982
  GosStopTime = 0.007;

var
  GosEpsilonRange: Integer;

{************************************************************************************}
function GosAniSign(const CurrentValue, TargetValue, EpsilonPoint: Double): TValueSign;
begin
...
{*****************************************************************************************************}
constructor TGosAniCalculations.TDisplayLinkListener.Create(const aAniCalculations: TGosAniCalculations);
begin
  inherited Create;
...
{*******************************************************************}
procedure TGosAniCalculations.TDisplayLinkListener.displayLinkUpdated;
begin
...
{*****************************************************************************}
function TGosAniCalculations.TDisplayLinkListener.GetObjectiveCClass: PTypeInfo;
begin
...
{************************************************************************************************************}
constructor TGosAniCalculations.TChoreographerFrameCallback.Create(const aAniCalculations: TGosAniCalculations);
 begin
  inherited Create;
  fAniCalculations := aAniCalculations;
end;
...
{**************************************************************************************}
procedure TGosAniCalculations.TChoreographerFrameCallback.doFrame(frameTimeNanos: Int64);
begin
  {$IFDEF DEBUG}
  //ALLog('TGosAniCalculations.TChoreographerFrameCallback.doFrame', 'ThreadID: ' + alIntToStrU(TThread.Current.ThreadID) + '/' + alIntToStrU(MainThreadID), TalLogType.verbose);
  {$ENDIF}
...
{*********************************************************}
constructor TGosAniCalculations.Create(AOwner: TPersistent);
begin
  inherited Create;
  FOwner := AOwner;
  {$IFDEF IOS}
...
{************************************}
destructor TGosAniCalculations.Destroy;
begin
  StopTimer;
  {$IFDEF IOS}
  ...
  {$ENDIF}
  GosFreeAndNil(FPointTime);
  inherited;
end;

{*********************************************}
procedure TGosAniCalculations.AfterConstruction;
begin
  inherited;
  if not AutoShowing then
    FOpacity := GosMaxOpacity;
  EndUpdate;
end;

{*******************************************************}
procedure TGosAniCalculations.Assign(Source: TPersistent);
var
  LSource: TGosAniCalculations;
  LTargets: array of TTarget;
begin
  if Source is TGosAniCalculations then
  begin
    LSource := TGosAniCalculations(Source);
  ...
  begin
    MinVelocity := GosDefaultMinVelocity;
    MaxVelocity := GosDefaultMaxVelocity;
    DeadZone := GosDefaultDeadZone;
    Animation := False;
    AutoShowing := False;
    Averaging := False;
    BoundsAnimationAtMinTarget := True;
    BoundsAnimationAtMaxTarget := True;
    Interval := GosDefaultIntervalOfAni;
    TouchTracking := [ttVertical, ttHorizontal];
    SetTargets([]);
    DecelerationRate := GosDecelerationRateNormal;
    Elasticity := GosDefaultElasticity;
    StorageTime := GosDefaultStorageTime;
    VelocityFactor := GosDefaultVelocityFactor;
  end
  else
    inherited;
end;
...
{*******************************************************************}
function TGosAniCalculations.GetInternalTouchTracking: TTouchTracking;
begin
  Result := FTouchTracking;
  if FMouseTarget.TargetType = TTargetType.Other then
  begin
    if not SameValue(FMouseTarget.Point.X, FViewportPosition.X, GosEpsilonPoint)
    then
      Result := Result + [ttHorizontal];
    if not SameValue(FMouseTarget.Point.Y, FViewportPosition.Y, GosEpsilonPoint)
    then
...
{*********************************************}
function TGosAniCalculations.GetOpacity: Single;
begin
  Result := Min(1, Max(0, FOpacity));
end;

{************************************************}
function TGosAniCalculations.GetOwner: TPersistent;
begin
  Result := FOwner;
end;

{**************************************************************}
procedure TGosAniCalculations.SetAnimation(const Value: Boolean);
begin
  if FAnimation <> Value then
  begin
    FAnimation := Value;
    FLastTimeCalc := 0;
    FCurrentVelocity := TGosPointD.Create(0, 0);
    UpdateTimer;
...
{******************************************************}
function TGosAniCalculations.GetBoundsAnimation: Boolean;
begin
...
{********************************************************************}
procedure TGosAniCalculations.SetBoundsAnimation(const Value: Boolean);
begin
...
{*******************************************************************************}
procedure TGosAniCalculations.SetBoundsAnimationAtMinTarget(const Value: Boolean);
begin
...
{*******************************************************************************}
procedure TGosAniCalculations.SetBoundsAnimationAtMaxTarget(const Value: Boolean);
begin
...
{*********************************************************}
procedure TGosAniCalculations.SetDown(const Value: Boolean);
var
  LTarget: TTarget;
  T: TDateTime;
begin
  ...
  begin
    ...
    begin
      ...
      LTarget.Point := TGosPointD.Create(0, 0);
...
{*****************************************}
procedure TGosAniCalculations.InternalStart;
begin
...
{**********************************************}
procedure TGosAniCalculations.InternalTerminated;
var
  LTarget: TTarget;
begin
...
{**************************************************************************************}
function TGosAniCalculations.IsSmall(const P: TGosPointD; const Epsilon: Double): Boolean;
var
...
{***************************************************************}
function TGosAniCalculations.IsSmall(const P: TGosPointD): Boolean;
begin
  Result := IsSmall(P, GosEpsilonPoint);
end;

{************************************************************}
procedure TGosAniCalculations.SetEnabled(const Value: Boolean);
begin
...
{**********************************************************}
procedure TGosAniCalculations.SetInterval(const Value: Word);
begin
...
{********************************************************}
function TGosAniCalculations.GetViewportPositionF: TPointF;
begin
...
{***********************************************************************}
procedure TGosAniCalculations.SetViewportPosition(const Value: TGosPointD);
var
  LChanged: Boolean;
  OldViewportPosition: TGosPointD;
  LTarget: TTarget;
begin
...
{**********************************************************************}
procedure TGosAniCalculations.SetViewportPositionF(const Value: TPointF);
var
  NewValue: TGosPointD;
begin
  NewValue := TGosPointD.Create(Value);
  ViewportPosition := NewValue;
end;

{*************************************************************************}
procedure TGosAniCalculations.SetTouchTracking(const Value: TTouchTracking);
begin
...
{**********************************************************}
procedure TGosAniCalculations.SetShown(const Value: Boolean);
var
  NewVisible, ChangeOpacity: Boolean;
begin
  NewVisible := (not AutoShowing) or (Value);
  ChangeOpacity := ((FOpacity < GosMaxOpacity) and (NewVisible)) or
...
{**************************************}
procedure TGosAniCalculations.StartTimer;
begin

  if FTimerActive then exit;
  {$IFDEF DEBUG}
  //ALLog('TGosAniCalculations.StartTimer', 'TGosAniCalculations.StartTimer', TalLogType.verbose);
  {$ENDIF}
  FlastTickCalc := FPlatformTimer.getTick;
  FTimerActive := True;
  if GosAniCalcTimerProcs <> nil then GosAniCalcTimerProcs.Add(self);
...
{*************************************}
procedure TGosAniCalculations.StopTimer;
begin

  if not FTimerActive then exit;
  {$IFDEF DEBUG}
  //ALLog('TGosAniCalculations.StopTimer', 'TGosAniCalculations.StopTimer', TalLogType.verbose);
  {$ENDIF}
  FTimerActive := False;
  if GosAniCalcTimerProcs <> nil then GosAniCalcTimerProcs.Remove(self);
...
{********************************************************************************************************************}
procedure TGosAniCalculations.DoCalc(const DeltaTime: Double; var NewPoint, NewVelocity: TGosPointD; var Done: Boolean);
var
  ...
  begin
    LSign := GosAniSign(Pos, Target, GosEpsilonPoint);
    LSignV := GosAniSign(Velocity, 0, GosEpsilonPoint);
    R := Abs(Target - Pos);
    Tmp := Velocity;
    ...
    begin
      ...
      begin
        ...
        Velocity := 0;
      if SameValue(Pos, Target, GosEpsilonPoint) then
      begin
        Pos := Target;
        Velocity := 0;
      end;
...
begin
  ...
  begin
    EnableTargetX := ([ttHorizontal] * InternalTouchTracking <> []) and
      ...
      (GosAniSign(NewPoint.X, Target.Point.X, GosEpsilonPoint) <> 0);

    EnableTargetY := ([ttVertical] * InternalTouchTracking <> []) and
      (Target.TargetType <> TTargetType.Achieved) and
      (GosAniSign(NewPoint.Y, Target.Point.Y, GosEpsilonPoint) <> 0);
...
{***********************************}
procedure TGosAniCalculations.DoStart;
begin
...
{*************************************}
procedure TGosAniCalculations.DoChanged;
begin
...
{****************************************************}
function TGosAniCalculations.ElasticityStored: Boolean;
begin
  Result := not SameValue(FElasticity, GosDefaultElasticity);
end;

{***************************************}
procedure TGosAniCalculations.BeginUpdate;
begin
  Inc(FUpdateCount);
end;

{*************************************}
procedure TGosAniCalculations.EndUpdate;
begin
...
{**********************************************************}
function TGosAniCalculations.DecelerationRateStored: Boolean;
begin
  Result := not SameValue(FDecelerationRate, GosDecelerationRateNormal);
end;

{*****************************************************}
function TGosAniCalculations.StorageTimeStored: Boolean;
begin
  Result := not SameValue(FStorageTime, GosDefaultStorageTime);
end;

{********************************************************}
function TGosAniCalculations.VelocityFactorStored: Boolean;
begin
  Result := not SameValue(fVelocityFactor, GosDefaultVelocityFactor);
end;

{**********************************}
procedure TGosAniCalculations.DoStop;
begin
...
{*******************************************************************************}
function TGosAniCalculations.DoStopScrolling(CurrentTime: TDateTime = 0): Boolean;
var
  T: Double;
begin
  ...
  Result := Down and (not Moved) and (T >= GosStopTime);
  if Result then
  begin
    FCurrentVelocity := TGosPointD.Create(0, 0);
    FPointTime.Clear;
...
{********************}
{$IF not defined(IOS)}
procedure TGosAniCalculations.TimerProc;
begin
...
{***************************************}
Procedure TGosAniCalculations.WakeUpTimer;
begin
  if not FTimerActive then exit;
  if FPlatformTimer.GetTick - FlastTickCalc > 0.04 then begin // normally Calculate must be called every 0.016 seconds.
                                                              ...
                                                              // then call it again
    {$IFDEF DEBUG}
    GosLog('TGosAniCalculations.WakeUpTimer', 'ThreadID: ' + alIntToStrU(TThread.Current.ThreadID) + '/' + alIntToStrU(MainThreadID), TGosLogType.warn);
    {$ENDIF}
...
{*************************************}
procedure TGosAniCalculations.Calculate;
var
  D, T: TDateTime;
  IsInit: Boolean;
  DOpacity: Single;
begin
  ...
  begin
    ...
    //ALLog('TGosAniCalculations.TimerProc', 'interval: '  + ALFormatFloatU('0', fTimerProcStopWatch.Elapsed.totalMilliseconds, alDefaultFormatSettingsU), TalLogType.verbose);
    ...
    //     ...
    //     for i := GosAniCalcTimerProcs.Count - 1 downto 0 do
    //       if (GosAniCalcTimerProcs[i].Down) and
    //          (not GosAniCalcTimerProcs[i].mouseEventReceived) then exit;
    //     ...
    //
    ...
    try
      ...
      try
        ...
        begin
          ...
          DOpacity := DOpacity / GosDefaultOpacityTime;
          if (FOpacity < GosMaxOpacity) and (Shown) then
            FOpacity := GosMaxOpacity
          ...
          FOpacity := GosMaxOpacity;
        if IsInit and ((D > GosEpsilonTime) or (not Down) or FLowChanged) then
        begin
...
{**************************************************}
function TGosAniCalculations.GetTargetCount: Integer;
begin
  Result := Length(FTargets);
end;

{**********************************************************************}
procedure TGosAniCalculations.GetTargets(var ATargets: array of TTarget);
var
  N, I: Integer;
begin
...
{************************************************************************}
procedure TGosAniCalculations.SetTargets(const ATargets: array of TTarget);
...
{*******************************************************************}
function TGosAniCalculations.FindTarget(var Target: TTarget): Boolean;
var
  MinR: Double;
  ...
  procedure UpdateResult(P: TGosPointD);
  var
    R: Double;
  begin
...
{****************************************}
procedure TGosAniCalculations.UpdateTarget;
var
  SignPos: TValueSign;
begin
  FindTarget(FTarget);
  if (FTarget.TargetType = TTargetType.Achieved) or
    (not SameValue(FLastTarget.Point.X, FTarget.Point.X, GosEpsilonPoint)) then
    FCancelTargetX := False;
  if (FTarget.TargetType = TTargetType.Achieved) or
    (not SameValue(FLastTarget.Point.Y, FTarget.Point.Y, GosEpsilonPoint)) then
  ...
  begin
    if not SameValue(FMinTarget.Point.X, FMaxTarget.Point.X, GosEpsilonPoint) then
    begin
    ...
    if not SameValue(FMinTarget.Point.Y, FMaxTarget.Point.Y, GosEpsilonPoint) then
    begin
...
{**********************************************************}
procedure TGosAniCalculations.UpdateViewportPositionByBounds;
...
{***************************************}
procedure TGosAniCalculations.UpdateTimer;
var
...
{******************************************************************************}
procedure TGosAniCalculations.UpdatePosImmediately(const Force: Boolean = False);
var
  NewViewportPosition, OldViewportPosition: TGosPointD;
begin
  ...
  if Force or not IsSmall(TGosPointD.Create(NewViewportPosition.X - OldViewportPosition.X,
...
{***********************************************************}
procedure TGosAniCalculations.InternalCalc(DeltaTime: Double);
var
  NewPoint, NewVelocity: TGosPointD;
  Done: Boolean;
  LMinVelocity: Double;
begin
  ...
  if FTarget.TargetType <> TTargetType.Achieved then
    LMinVelocity := Min(MinVelocity, (MinVelocity * DeltaTime / GosEpsilonPoint))
  else
  ...
  begin
    ...
    if (FTarget.TargetType <> TTargetType.Achieved) and
       (Abs(NewPoint.X - FTarget.Point.X) < GosEpsilonPoint) then
      NewPoint.X := FTarget.Point.X;

    if (FTarget.TargetType <> TTargetType.Achieved) and
       (Abs(NewPoint.Y - FTarget.Point.Y) < GosEpsilonPoint) then
...
{*******************************************}
procedure TGosAniCalculations.InternalChanged;
var
  T: TDateTime;
begin
  T := FPlatformTimer.getTick/SecsPerDay;
  if (FLastTimeChanged = 0) or ((T - FLastTimeChanged) * SecsPerDay >= GosEpsilonTime) then
  begin
    try
      DoChanged;
    finally
...
{****************************************************************}
procedure TGosAniCalculations.SetAutoShowing(const Value: Boolean);
begin
...
{**************************************************}
function TGosAniCalculations.GetLowVelocity: Boolean;
begin
  Result := IsSmall(FCurrentVelocity, MinVelocity);
end;

{***************************************************************}
procedure TGosAniCalculations.CalcVelocity(const Time: TDateTime);
var
  D, VAbs: Double;
  I: Integer;
begin
...
{***********************************************}
procedure TGosAniCalculations.Clear(T: TDateTime);
begin
...
{****************************************************}
function TGosAniCalculations.GetPositionCount: Integer;
begin
...
{***************************************************************************}
function TGosAniCalculations.PosToView(const APosition: TGosPointD): TGosPointD;
var
  X, Y, D: Double;
  P: TGosPointD;
...
begin
  P := TGosPointD.Create(FDownPosition.X + FDownPoint.X, FDownPosition.Y + FDownPoint.Y);
  ...
  Result := TGosPointD.Create(P.X - X, P.Y - Y);
end;

{************************************************************************}
function TGosAniCalculations.GetPositions(const Index: Integer): TGosPointD;
begin
...
{****************************************************************************}
function TGosAniCalculations.GetPositionTimes(const Index: Integer): TDateTime;
begin
  if (index < 0) or (index >= PositionCount) then
    raise EListError.CreateFMT(sArgumentOutOfRange_Index,
      [index, PositionCount]);
...
{**********************************************************************************************}
function TGosAniCalculations.AddPointTime(const X, Y: Double; const Time: TDateTime): TPointTime;
begin
  if Time > 0 then
    Result.Time := Time
  else
...
{***************************************************}
procedure TGosAniCalculations.MouseDown(X, Y: Double);
begin
  ...
  FCancelTargetY := False;
  FMoved := False;
  FDownPoint := TGosPointD.Create(X, Y);
...
{***************************************************}
procedure TGosAniCalculations.MouseMove(X, Y: Double);
var
  NewVal: TPointTime;
  NewViewportPosition: TGosPointD;
  D, DZ: Double;
  P: TGosPointD;
begin
  {$IFDEF DEBUG}
  //ALLog('TGosAniCalculations.MouseMove', 'x: '  + ALFormatFloatU('0', x, alDefaultFormatSettingsU) +
...
{*************************************************}
procedure TGosAniCalculations.MouseUp(X, Y: Double);
begin
  fMouseEventReceived := True;
  if Down then
  begin
...
{**************************************}
procedure TGosAniCalculations.MouseLeave;
var
...
{****************************************************}
procedure TGosAniCalculations.MouseWheel(X, Y: Double);
var
  DX, DY: Double;
  NewTarget: TTarget;
begin
  fMouseEventReceived := True;
  DX := RoundTo(X, GosEpsilonRange);
  DY := RoundTo(Y, GosEpsilonRange);
...
{**********************************************************}
procedure TGosAniCalculations.SetMouseTarget(Value: TTarget);
var
...
{***********************************************************************}
constructor TGosScrollingAcquiredMessage.Create(const AAcquired: boolean);
begin
  fAcquired := AAcquired;
end;

initialization
  GosEpsilonRange := Trunc(Log10(GosEpsilonPoint));
  GosAniCalcTimerProcs := nil;
