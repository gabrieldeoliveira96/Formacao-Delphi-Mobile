...
{$SCOPEDENUMS ON}
uses
  uGosDebug, uGosUtils, uGosBase, uGosStandard, uGosUtils.ArrayEx, uGosAni,
  {$IFDEF MSWINDOWS}
...
type
  ...
  IListAdapter = interface
    ...
    function GetView(const Index: Integer; ConvertView: TGosViewBase; Parent: TGosViewGroup): TGosViewBase;
  ...
  TListDividerView = class(TGosView);
  TListTextItem = class(TGosTextView)
  private const
    C_MinHeight = 48;
    C_FontSize = 15;
  end;
  ...
  TListViewList = TList<TGosViewBase>;
  /// <summary>
  /// 列表项调节时的状态数据
  /// </summary>
  ...
  /// 列表视图内容区域
  /// </summary>
  TListViewContent = class(TGosView)
  private
    [Weak] ListView: TListViewEx;
    [Weak] FAdapter: IListAdapter;
    FIsDesigning: Boolean;                    // 是否为设计模式
    FViews: TDictionary<Integer, TGosViewBase>;  // 当前显示的控件列表
    ...
    function GetControlFormCacle(const ItemType: Integer): TGosViewBase;
    procedure AddControlToCacle(const ItemType: Integer; const Value: TGosViewBase);
  ...
  public
    ...
    function AddHeaderView(const View: TGosViewClass): TControl; overload;
    /// <summary>
    /// 删除添加在列表头部的View
    /// </summary>
    ...
    function AddFooterView(const View: TGosViewClass): TControl; overload;
    /// <summary>
    /// 删除添加在列表底部的View
    /// </summary>
  ...
  TListViewEx = class(TGosScrollView)
  private const
  ...
  protected
    ...
    function GetView(const Index: Integer; ConvertView: TGosViewBase; Parent: TGosViewGroup): TGosViewBase; virtual; abstract;
  public
    constructor Create();
  ...
  protected
    ...
    function GetItem(const Index: Integer): Pointer; override;
    function IndexOf(const AItem: Pointer): Integer; override;
    function GetView(const Index: Integer; ConvertView: TGosViewBase; Parent: TGosViewGroup): TGosViewBase; override;
...
type
  ...
  protected
    ...
    function GetView(const Index: Integer; ConvertView: TGosViewBase;
      Parent: TGosViewGroup): TGosViewBase; override;
    function GetNodeGroupView(const Index: Integer; const ANode: TTreeListNode<T>;
      ConvertView: TGosViewBase; Parent: TGosViewGroup): TGosViewBase; virtual;
...
implementation
uses
  uGosListView.Header, uGosListView.Footer, uGosListView.TreeGroup;
{ TListViewEx }
...
function TListViewEx.AddFooterView(const View: TGosViewClass): TControl;
begin
  Result := FContentViews.AddFooterView(View);
end;
function TListViewEx.AddHeaderView(const View: TGosViewClass): TControl;
begin
...
function TListViewEx.CanRePaintBk(const View: IGosView;
...
function TListViewContent.AddFooterView(const View: TGosViewClass): TControl;
begin
...
function TListViewContent.AddHeaderView(const View: TGosViewClass): TControl;
begin
  if not Assigned(View) then
    Result := nil
  else begin
...
var
  ...
  ItemView: TPair<Integer, TGosViewBase>;
  I: Integer;
begin
...
begin
  ...
  FViews := TDictionary<Integer, TGosViewBase>.Create(256);
...
begin
  if FItemViews.TryGetValue(THashType(Sender), ItemIndex) then begin
    ...
    if Assigned(ListView.FOnItemClick) then
      ListView.FOnItemClick(ListView, ItemIndex,  TGosView(FViews[ItemIndex]));
  end;
end;
...
procedure TListViewContent.DoRealign;
  ...
  // 获取 View
  function GetView(const ItemView: TGosViewBase): TGosView;
  begin
    if ItemView is TGosView then begin
      Result := ItemView as TGosView;
    end else if (TControl(ItemView) is TFrame) and (ItemView.ControlsCount = 1) and (ItemView.Controls[0] is TGosView) then
      Result := TGosView(ItemView.Controls[0]) // 如果是 Frame ，且只有一个TView子组件
    else
      Result := nil;
  end;
  // 移除列表项, 归还到缓存中
  procedure RemoveItemView(const I: Integer; const ItemView: TGosViewBase);
  begin
    FViews.Remove(I);
  ...
  begin
    ...
    if FViews.ContainsKey(I) then begin
      ...
      if FAdapter.DisableCache() then
        ItemView := FAdapter.GetView(I, FViews[I], TGosViewGroup(Self))
      else
        ItemView := FViews[I];
      // 如果返回nil, 抛出错误
      ...
      // 获取一个列表项视图
      ItemView := FAdapter.GetView(I, Control, TGosViewGroup(Self));
      // 如果返回nil, 抛出错误
      if not Assigned(ItemView) then
        raise Exception.Create('View is null.');
      // 记录到可视列表中
...
function TListViewContent.GetControlFormCacle(const ItemType: Integer): TGosViewBase;
var
  List: TListViewList;
begin
  if not FCacleViews.TryGetValue(ItemType, List) then begin
...
function TStringsListAdapter.GetView(const Index: Integer;
  ConvertView: TGosViewBase; Parent: TGosViewGroup): TGosViewBase;
var
...
begin
  if (ConvertView = nil) or (not (TControl(ConvertView) is TListViewItemCheck)) then begin
    ...
    ViewItem.Gravity := TLayoutGravity.CenterVertical;
    ViewItem.Width := Parent.Width;
    ViewItem.TextView1 := TGosTextView.Create(ViewItem);
...
function TCustomTreeListDataAdapter<T>.GetNodeGroupView(const Index: Integer;
  const ANode: TTreeListNode<T>; ConvertView: TGosViewBase;
  Parent: TGosViewGroup): TGosViewBase;
var
