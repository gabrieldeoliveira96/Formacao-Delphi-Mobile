...
uses
  uGosBase, uGosAni, uGosUtils, uGosStandard, uGosListView,
...
type
  TButtonViewColor = class(TGosViewColor)
  public
...
type
  ...
  protected
    ...
    FTitleView: TGosTextView;
    FTitleSpace: TGosView;
    FMsgBody: TLinearLayout;
    FMsgMessage: TGosTextView;
    FButtonLayout: TLinearLayout;
    FButtonPositive: TGosButtonView;
    FButtonNegative: TGosButtonView;
    FButtonNeutral: TGosButtonView;
    FCancelButtonLayout: TLinearLayout;
    FButtonCancel: TGosButtonView;
  ...
  public
    ...
    property TitleView: TGosTextView read FTitleView;
    property MessageView: TGosTextView read FMsgMessage;
    property ButtonLayout: TLinearLayout read FButtonLayout;
    property ButtonPositive: TGosButtonView read FButtonPositive;
    property ButtonNegative: TGosButtonView read FButtonNegative;
    property ButtonNeutral: TGosButtonView read FButtonNeutral;
    property CancelButtonLayout: TLinearLayout read FCancelButtonLayout;
    property ButtonCancel: TGosButtonView read FButtonCancel;
  ...
  public
    ...
    /// 设置图标
    /// </summary>
    function SetIcon(AIcon: TGosDrawableBase): TDialogBuilder; overload;
    /// <summary>
    /// 设置图标
    /// </summary>
...
function TDialogBuilder.SetIcon(AIcon: TGosDrawableBase): TDialogBuilder;
begin
  Result := Self;
  FIcon := AIcon;
end;
...
begin
  ...
  if (not Assigned(AniView)) or (not FMask) or
    ((AniView is TGosView) and (TGosView(AniView).Background.ItemDefault.Color and $FF000000 = 0)) then begin
...
begin
  ...
  {$IFDEF ANDROID}
  Dialog.FViewRoot.Padding.Top := TGosView.GetStatusHeight;
  {$ENDIF}
...
begin
  ...
  if (Builder.FIcon <> nil) or (Builder.FMessage <> '') then begin
    ...
    if Assigned(Builder.FIcon) then begin
      if Builder.FIcon is TGosDrawableBase then
        FViewRoot.FMsgMessage.Drawable.Assign(TGosDrawableBase(Builder.FIcon))
...
var
  FView1, FView2: TGosButtonView;

  procedure SetButton(var B: TGosButtonView; FText: string;
  ...
  procedure SetButtonColor(Button: TGosButtonView; State: TViewState);
  var
    AColor: TAlphaColor;
    ABrush: TBrush;
  begin
  ...
  function CreateButton(Parent: TFmxObject): TGosButtonView;
  begin
    Result := TGosButtonView.Create(Owner);
...
begin
  ...
  // 内容区
  FMsgMessage := TGosTextView.Create(Owner);
  {$IFDEF MSWINDOWS}
...
begin
  ...
  {$IFDEF ANDROID}
  FLayBubble.Margins.Top := FLayBubble.Margins.Top + TGosView.GetStatusHeight;
  {$ENDIF}
...
begin
  ...
  // 标题栏
  FTitleView := TGosTextView.Create(Owner);
  {$IFDEF MSWINDOWS}
  ...
  if StyleMgr.FTitleSpaceHeight > 0 then begin
    FTitleSpace := TGosView.Create(Owner);
    {$IFDEF MSWINDOWS}
