...
uses
  uGosDebug, uGosUtils, uGosUtils.SVGImage,
  FMX.Forms,
  FMX.FontGlyphs,
  {$IFDEF ANDROID}
  Androidapi.Helpers,
...
const
  AllCurrentPlatforms =
    ...
    pidiOSSimulator or pidiOSDevice or pidAndroid or pidAndroid64Arm;

type
  IGosView = interface;
  IGosViewGroup = interface;
  TGosView = class;
  TGosViewGroup = class;

  TGosDrawableBase = class;
  TGosDrawable = class;
  TGosDrawableIcon = class;
  TGosViewColor = class;
  TGosDrawableBrush = class;

  EViewError = class(Exception);
  EViewLayoutError = class(Exception);
  EDrawableError = class(Exception);

  TGosViewClass = class of TControl;
  ...
  /// 可绘制对象
  /// </summary>
  TGosDrawableBase = class(TPersistent)
  private
    FOnChanged: TNotifyEvent;
    FKind: TDrawableKind;
  ...
  public
    constructor Create(View: IGosView; const ADefaultKind: TViewBrushKind = TViewBrushKind.None;
    ...
    procedure SetDrawable(const Value: TGosDrawableBase); overload;
    procedure SetBrush(State: TViewState; const Value: TBrush); overload;
    procedure SetBrush(State: TViewState; const Value: TGosDrawableBrush); overload;
  ...
  /// 可绘制对象
  /// </summary>
  TGosDrawable = class(TGosDrawableBase)
  private
    FPadding: TBounds;
  ...
  private
    FOnChanged: TNotifyEvent;
    FBrush: TStrokeBrush;
    FColor: TGosViewColor;
    FStyle: TViewBorderStyle;
    FDefaultStyle: TViewBorderStyle;
    FLines: TViewLines;
    procedure SetColor(const Value: TGosViewColor);
  ...
  published
    property Color: TGosViewColor read FColor write SetColor;
  ...
  TDrawableBorder = class(TGosDrawable)
  private
    FBorder: TViewBorder;
  ...
  public
    constructor Create(AOwner: TGosDrawableIcon); reintroduce;
  ...
  TGosDrawableBrush = class(TComponent, IGlyph, IInterfaceComponentReference)
  private
    FBrush: TBrush;
  ...
  /// 可绘制图标
  /// </summary>
  TGosDrawableIcon = class(TGosDrawableBase, IInterface, IGlyph, IInterfaceComponentReference)
  private
    FWidth: Integer;
    FHeight: Integer;
    FPadding: Integer;
  ...
  /// 颜色属性
  /// </summary>
  TGosViewColor = class(TPersistent)
  private
    FOnChanged: TNotifyEvent;
  ...
  TTextColor = class(TGosViewColor)
  private
  ...
  public
    constructor Create(View: IGosView);
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  ...
  /// 视图布局属性接口
  /// </summary>
  IGosView = interface(IInterface)
    ['{9C2D9DB0-9D59-4A9D-BC47-53928194544E}']
    function GetBackground: TGosDrawable;
    function GetLayout: TViewLayout;
    function GetParentControl: TControl;
    function GetParentView: IGosViewGroup;
    ...
    procedure SetBackground(const Value: TGosDrawable);
    ...
    property Background: TGosDrawable read GetBackground write SetBackground;
    ...
    property ParentView: IGosViewGroup read GetParentView;
  ...
  /// 视图组接口
  /// </summary>
  IGosViewGroup = interface(IGosView)
    ['{73A1B9E5-D4AF-4956-A15F-73B0B8EDADF9}']
    function AddView(View: TGosView): Integer;
    function RemoveView(View: TGosView): Integer;
  ...
  public
    ...
    // 绘制到画布中
    procedure Draw(Canvas: TCanvas; TextSet: uGosBase.TTextSettings; const R: TRectF;
      const Opacity: Single; State: TViewState; ASize: PSizeF = nil);

    // 计算大小
    procedure CalcTextSize(Canvas: TCanvas; TextSet: uGosBase.TTextSettings; const R: TRectF;
      var ASize: TSizeF);

    procedure MouseDown(Sender: TGosView; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
    procedure MouseMove(Sender: TGosView; X, Y: Single);
    procedure MouseUp(Sender: TGosView; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
    procedure MouseLeave(Sender: TGosView);
  ...
  public
    ...
    property ScrollView: TGosView read FScrollView;
  end;
  ...
  TGosViewBase = class(TControl)
  protected
    function GetBackground: TGosDrawable; virtual;
    function GetViewBackground: TGosDrawable; virtual;
    ...
    procedure SetBackgroundBase(const Value: TGosDrawable); virtual; abstract;
  ...
  public
    property Background: TGosDrawable read GetViewBackground write SetBackgroundBase;
    property MinWidth: Single read GetMinWidth write SetMinWidth;
    property MinHeight: Single read GetMinHeight write SetMinHeight;
    property MaxWidth: Single read GetMaxWidth write SetMaxWidth;
    property MaxHeight: Single read GetMaxHeight write SetMaxHeight;
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TGosView = class(TGosViewBase, IGosView)
  const
  ...
  protected
    ...
    function GetBackground: TGosDrawable; override;
    function GetViewBackground: TGosDrawable; override;
    ...
    procedure SetBackgroundBase(const Value: TGosDrawable); override;
  protected
    FWeight: Single;
    FInVisible: Boolean;
    FGravity: TLayoutGravity;
    FOrientation: TOrientation;
    FBackground: TGosDrawable;
    FDrawing: Boolean;
    ...
    function GetBadgeView: IViewBadge;
    procedure SetBadgeView(const Value: IViewBadge);
    function EmptyBackground(const V: TGosDrawable; const State: TViewState): Boolean;
    function CanRePaintBk(const View: IGosView; State: TViewState): Boolean; virtual;
  ...
  protected
    ...
    procedure ImagesChanged; virtual;
    function CreateBackground: TGosDrawable; virtual;
  ...
  public
    ...
    procedure SetBackground(const Value: TGosDrawable); overload;
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TGosViewGroup = class(TGosView, IGosViewGroup)
  private
  protected
    /// <summary>
    /// 是否需要自动调整大小
    /// </summary>
    function IsAdjustSize(View: IGosView; Align: TAlignLayout;
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TLinearLayout = class(TGosViewGroup)
  private
  protected
    /// <summary>
    /// 查找最后一个需要自动调整大小的组件
    ...
    function GetLastWeightView(): TGosView;
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TRelativeLayout = class(TGosViewGroup)
  private
  ...
  /// </summary>
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TGridsLayout = class(TGosViewGroup)
  private const
...
procedure CheckView(const List: TInterfaceList; const View: IGosView);

  procedure Check(const Control: TControl);
  var
    View: IGosView;
  begin
    if Supports(Control, IGosView, View) then
      CheckView(List, View);
  end;
...
function CheckRecursionState(const Control: IGosView): Boolean;
var
  List: TInterfaceList;
begin
...
procedure TGosDrawableBase.Assign(Source: TPersistent);

  procedure AssignItem(State: TViewState; const Src: TGosDrawableBase);
  var V: TBrush;
  begin
...
var
  ...
  Src: TGosDrawable;
begin
  if Source is TGosDrawableBase then begin
    SaveChange := FOnChanged;
    FOnChanged := nil;
    Src := TGosDrawable(Source);
...
function TGosDrawableBase.BrushIsEmpty(V: TBrush): Boolean;
begin
...
procedure TGosDrawableBase.Change;
begin
  DoChange(Self);
end;

constructor TGosDrawableBase.Create(View: IGosView; const ADefaultKind: TViewBrushKind;
...
procedure TGosDrawableBase.CreateBrush(var Value: TBrush;
...
function TGosDrawableBase.CreateBrush: TBrush;
begin
  CreateBrush(Result);
end;

destructor TGosDrawableBase.Destroy;
begin
  FOnChanged := nil;
  FreeAndNil(FDefault);
  FreeAndNil(FPressed);
...
procedure TGosDrawableBase.DoChange(Sender: TObject);
begin
  FIsEmpty := GetEmpty;
  if Assigned(FOnChanged) then
    FOnChanged(Sender);
...
procedure TGosDrawableBase.DoDrawed(Canvas: TCanvas; var R: TRectF; AState: TViewState; const AOpacity: Single);
begin
end;

function TGosDrawableBase.GetValue(const Index: Integer): TBrush;
begin
  Result := GetBrush(TViewState(Index), not (csLoading in FView.GetComponentState));
end;

procedure TGosDrawableBase.InitDrawable;
begin
end;

function TGosDrawableBase.IsStoredCorners: Boolean;
begin
  Result := FCorners <> AllCorners;
end;

function TGosDrawableBase.GetBrush(const State: TViewState; AutoCreate: Boolean): TBrush;
begin
...
function TGosDrawableBase.GetDrawRect(const ALeft, ATop, ARight, ABottom: Single): TRectF;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
  Result.Right := ARight;
...
function TGosDrawableBase.GetEmpty: Boolean;
begin
...
function TGosDrawableBase.GetStateBrush(const State: TViewState): TBrush;
begin
  GetStateBrush(State, Result);
end;

procedure TGosDrawableBase.GetStateBrush(const State: TViewState; var V: TBrush);
begin
  case State of
...
function TGosDrawableBase.GetStateImagesItem(AState: TViewState): TBrush;
begin
...
function TGosDrawableBase.GetStateItem(AState: TViewState): TBrush;
begin
  GetStateBrush(AState, Result);
  if Result <> FDefault then begin
    if BrushIsEmpty(Result) then
...
procedure TGosDrawableBase.Draw(Canvas: TCanvas);
var
  V: TBrush;
  R: TRectF;
  AState: TViewState;
...
procedure TGosDrawableBase.DrawBrushTo(Canvas: TCanvas; ABrush: TBrush;
  const R: TRectF);
begin
  if ABrush <> nil then
...
procedure TGosDrawableBase.DrawStateTo(Canvas: TCanvas; const R: TRectF;
  AState: TViewState);
begin
...
procedure TGosDrawableBase.DrawTo(Canvas: TCanvas; const R: TRectF);
begin
  if FIsEmpty or (not Assigned(FView)) then Exit;
  DrawStateTo(Canvas, R, FView.GetDrawState);
end;

procedure TGosDrawableBase.FillArc(Canvas: TCanvas; const Center, Radius: TPointF;
...
procedure TGosDrawableBase.FillRect(Canvas: TCanvas; const ARect: TRectF;
  const XRadius, YRadius: Single; const ACorners: TCorners;
...
class procedure TGosDrawableBase.FillRect9Patch(Canvas: TCanvas; const ARect: TRectF;
  const XRadius, YRadius: Single; const ACorners: TCorners;
...
procedure TGosDrawableBase.SetDrawable(const Value: TGosDrawableBase);
begin
  Assign(Value);
end;

procedure TGosDrawableBase.SetColor(State: TViewState; const Value: TAlphaColor);
var V: TBrush;
begin
...
procedure TGosDrawableBase.SetCorners(const Value: TCorners);
begin
...
procedure TGosDrawableBase.SetCornerType(const Value: TCornerType);
begin
...
procedure TGosDrawableBase.SetGradient(State: TViewState; const Value: TGradient);
var V: TBrush;
begin
  V := GetBrush(State, True);
...
procedure TGosDrawableBase.SetKind(const Value: TDrawableKind);
begin
...
procedure TGosDrawableBase.SetRadius(const X, Y: Single);
begin
  FYRadius := Y;
  FXRadius := X;
  DoChange(Self);
...
procedure TGosDrawableBase.SetBitmap(State: TViewState; const Value: TBrushBitmap);
var V: TBrush;
begin
  V := GetBrush(State, True);
...
procedure TGosDrawableBase.SetBrush(State: TViewState;
  const Value: TGosDrawableBrush);
var V: TBrush;
begin
  if not Assigned(Value) then Exit;
  V := GetBrush(State, True);
  if (Self is TGosDrawableIcon) and (Value.ImageIndex >= 0) then
    TGosDrawableIcon(Self).Images := Value.Images;
  V.Assign(Value.Brush);
end;

procedure TGosDrawableBase.SetBrush(State: TViewState; const Value: TBrush);
begin
  GetBrush(State, True).Assign(Value);
end;

procedure TGosDrawableBase.SetBitmap(State: TViewState; const Value: TBitmap);
var V: TBrush;
begin
  V := GetBrush(State, True);
...
procedure TGosDrawableBase.SetStateBrush(const State: TViewState; const V: TBrush);
begin
  case State of
...
procedure TGosDrawableBase.SetValue(const Index: Integer; const Value: TBrush);
begin
  SetBrush(TViewState(Index), Value);
end;

procedure TGosDrawableBase.SetXRadius(const Value: Single);
begin
...
procedure TGosDrawableBase.SetYRadius(const Value: Single);
begin
...
procedure TGosDrawable.Assign(Source: TPersistent);
var
  LastOnChange: TNotifyEvent;
begin
  if Source is TGosDrawable then begin
    LastOnChange := FPadding.OnChange;
    FPadding.OnChange := nil;
    FPadding.Assign(TGosDrawable(Source).FPadding);
    FPadding.OnChange := LastOnChange;
  end;
  inherited Assign(Source);
end;

constructor TGosDrawable.Create(View: IGosView; const ADefaultKind: TViewBrushKind;
  const ADefaultColor: TAlphaColor);
begin
...
destructor TGosDrawable.Destroy;
begin
  FreeAndNil(FPadding);
  inherited Destroy;
end;

function TGosDrawable.GetDrawRect(const ALeft, ATop, ARight, ABottom: Single): TRectF;
begin
...
function TGosDrawable.GetPaddings: string;
begin
  Result := GetBoundsFloat(FPadding);
end;

function TGosDrawable.GetValue(const Index: Integer): TViewBrush;
begin
  Result := inherited GetBrush(TViewState(Index),
    not (csLoading in FView.GetComponentState)) as TViewBrush;
end;

procedure TGosDrawable.SetPadding(const Value: TBounds);
begin
  FPadding.Assign(Value);
end;

procedure TGosDrawable.SetPaddings(const Value: string);
var
  V: Single;
begin
...
procedure TGosDrawable.SetValue(const Index: Integer; const Value: TViewBrush);
begin
...
procedure TGosDrawableIcon.AdjustDraw(Canvas: TCanvas; var R: TRectF; ExecDraw: Boolean;
  AState: TViewState);
var
  DR: TRectF;
  SW, SH: Single;
...
procedure TGosDrawableIcon.Assign(Source: TPersistent);
begin
  if Source is TGosDrawableIcon then begin
    FWidth := TGosDrawableIcon(Source).FWidth;
    FHeight := TGosDrawableIcon(Source).FHeight;
    FPadding := TGosDrawableIcon(Source).FPadding;
    FPosition := TGosDrawableIcon(Source).FPosition;
  end;
  inherited Assign(Source);
end;

constructor TGosDrawableIcon.Create(View: IGosView; const ADefaultKind: TViewBrushKind;
  const ADefaultColor: TAlphaColor);
begin
  FView := View;
...
procedure TGosDrawableIcon.CreateBrush(var Value: TBrush;
  const ADefaultKind: TViewBrushKind; const ADefaultColor: TAlphaColor);
begin
  if Assigned(Value) then
    FreeAndNil(Value);
...
destructor TGosDrawableIcon.Destroy;
begin
  FImageLink.DisposeOf;
  inherited;
end;

procedure TGosDrawableIcon.Draw(Canvas: TCanvas);
var
  ImageIndex: Integer;
begin
...
procedure TGosDrawableIcon.DrawImage(Canvas: TCanvas; Index: Integer;
  const R: TRectF);
begin
...
procedure TGosDrawableIcon.DrawStateTo(Canvas: TCanvas; const R: TRectF; AState: TViewState; const AOpacity: Single);
var
  ImageIndex: Integer;
begin
...
function TGosDrawableIcon.GetComponent: TComponent;
begin
  Result := FView.GetComponent;
end;

function TGosDrawableIcon.GetEmpty: Boolean;
begin
...
function TGosDrawableIcon.GetImageIndex: TImageIndex;
begin
  Result := FImageLink.ImageIndex;
end;

function TGosDrawableIcon.GetImageList: TBaseImageList;
begin
  Result := GetImages;
end;

function TGosDrawableIcon.GetImages: TCustomImageList;
begin
...
function TGosDrawableIcon.GetStateImageIndex: Integer;
begin
...
function TGosDrawableIcon.GetStateImageIndex(State: TViewState): Integer;
var
  V: TBrush;
begin
  Result := -1;
...
procedure TGosDrawableIcon.ImagesChanged;
begin
  DoChange(Self);
end;

function TGosDrawableIcon.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then Result := S_OK
  else Result := E_NOINTERFACE
end;

procedure TGosDrawableIcon.SetHeight(const Value: Integer);
begin
...
procedure TGosDrawableIcon.SetImageIndex(const Value: TImageIndex);
begin
  FImageLink.ImageIndex := Value;
end;

procedure TGosDrawableIcon.SetImageList(const Value: TBaseImageList);
begin
  ValidateInheritance(Value, TCustomImageList);
  SetImages(TCustomImageList(Value));
end;

procedure TGosDrawableIcon.SetImages(const Value: TCustomImageList);
begin
  FImageLink.Images := Value;
end;

procedure TGosDrawableIcon.SetPadding(const Value: Integer);
begin
...
procedure TGosDrawableIcon.SetPosition(const Value: TDrawablePosition);
begin
...
procedure TGosDrawableIcon.SetWidth(const Value: Integer);
begin
...
function TGosDrawableIcon._AddRef: Integer;
begin
  Result := -1;
end;

function TGosDrawableIcon._Release: Integer;
begin
  Result := -1;
end;
...
procedure TGosViewColor.Assign(Source: TPersistent);
var
  Src: TGosViewColor;
begin
  if Source = nil then begin
    ...
    if Assigned(FOnChanged) then
      FOnChanged(Self);
  end else if Source is TGosViewColor then begin
    Src := TGosViewColor(Source);
...
constructor TGosViewColor.Create(const ADefaultColor: TAlphaColor);
begin
...
function TGosViewColor.ColorActivatedStored: Boolean;
begin
  Result := GetColorStoreState(8);
end;

function TGosViewColor.ColorCheckedStored: Boolean;
begin
  Result := GetColorStoreState(6);
end;

function TGosViewColor.ColorDefaultStored: Boolean;
begin
  Result := GetColorStoreState(1);
end;

function TGosViewColor.ColorEnabledStored: Boolean;
begin
  Result := GetColorStoreState(7);
end;

function TGosViewColor.ColorFocusedStored: Boolean;
begin
  Result := GetColorStoreState(3);
end;

function TGosViewColor.ColorHoveredStored: Boolean;
begin
  Result := GetColorStoreState(4);
end;

function TGosViewColor.ColorPressedStored: Boolean;
begin
  Result := GetColorStoreState(2);
end;

function TGosViewColor.ColorSelectedStored: Boolean;
begin
  Result := GetColorStoreState(5);
end;

destructor TGosViewColor.Destroy;
begin
  inherited;
end;

procedure TGosViewColor.DoChange(Sender: TObject);
begin
  if Assigned(FOnChanged) then
    FOnChanged(Sender);
end;

function TGosViewColor.GetColor(State: TViewState): TAlphaColor;
begin
  case State of
...
function TGosViewColor.GetColorStoreState(const Index: Integer): Boolean;
begin
  Result := (FColorStoreState and Index) <> 0;
end;

function TGosViewColor.GetStateColor(State: TViewState): TAlphaColor;
begin
...
function TGosViewColor.GetValue(const Index: Integer): TAlphaColor;
begin
  Result := GetColor(TViewState(Index));
end;

procedure TGosViewColor.SetActivated(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetChecked(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetColor(State: TViewState; const Value: TAlphaColor);
begin
  case State of
...
procedure TGosViewColor.SetColorStoreState(const Index: Integer;
  const Value: Boolean);
begin
  if Value then
...
procedure TGosViewColor.SetDefault(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetEnabled(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetFocused(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetHovered(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetPressed(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetSelected(const Value: TAlphaColor);
begin
...
procedure TGosViewColor.SetValue(const Index: Integer; const Value: TAlphaColor);
begin
...
constructor TViewLayout.Create(View: IGosView);
begin
  FView := View;
end;
...
function TGosViewBase.GetBackground: TGosDrawable;
begin
  Result := nil;
end;

function TGosViewBase.GetMaxHeight: Single;
begin
  Result := 0;
end;

function TGosViewBase.GetMaxWidth: Single;
begin
  Result := 0;
end;

function TGosViewBase.GetMinHeight: Single;
begin
  Result := 0;
end;

function TGosViewBase.GetMinWidth: Single;
begin
  Result := 0;
end;

function TGosViewBase.GetViewBackground: TGosDrawable;
begin
  Result := nil;
end;

function TGosViewBase.GetViewStates: TViewStates;
begin
  Result := [];
...
procedure TGosView.AfterPaint;
begin
  inherited;
  FInvaliding := False;
end;

function TGosView.AllowUseLayout: Boolean;
begin
  Result := (not (csDesigning in ComponentState)) or
    (Assigned(ParentControl)) and (ParentControl is TRelativeLayout);
end;

procedure TGosView.ApplyTriggerEffect(const AInstance: TFmxObject;
  const ATrigger: string);
begin
  // inherited; disable all effect
end;

function TGosView.CanAnimation: Boolean;
begin
  Result := False;
end;

function TGosView.CanRePaintBk(const View: IGosView; State: TViewState): Boolean;
begin
  Result := CanRepaint and EmptyBackground(View.Background, State);
end;

function TGosView.EmptyBackground(const V: TGosDrawable;
...
constructor TGosView.Create(AOwner: TComponent);
begin
...
function TGosView.CreateBackground: TGosDrawable;
begin
  Result := TDrawableBorder.Create(Self);
  Result.OnChanged := DoBackgroundChanged;
end;

procedure TGosView.DecChildState(State: TViewState);
var
  I: Integer;
  View: IGosView;
begin
  ...
  for I := 0 to Controls.Count - 1 do begin
    if Supports(Controls.Items[I], IGosView, View) then
      View.DecViewState(State);
  end;
end;

procedure TGosView.DecViewState(const State: TViewState);
begin
...
procedure TGosView.DoActivate;
begin
  //IncViewState(TViewState.Activated);
  inherited DoActivate;
end;

procedure TGosView.DoAdjustViewBounds(var ANewWidth, ANewHeight: Single);
var
  AMaxW, AMaxH: Single;
begin
...
procedure TGosView.DoBackgroundChanged(Sender: TObject);
begin
  Repaint;
end;

procedure TGosView.DoChangeSize(var ANewWidth, ANewHeight: Single);
begin
  DoRecalcSize(ANewWidth, ANewHeight);
  DoAdjustViewBounds(ANewWidth, ANewHeight);
end;

procedure TGosView.DoCheckedChange;
begin
end;

procedure TGosView.DoClickEvent;
begin
end;

procedure TGosView.DoDeactivate;
begin
  DecViewState(TViewState.Activated);
  inherited DoDeactivate;
end;

procedure TGosView.DoEndUpdate;
begin
...
function TGosView.DoGetUpdateRect: TRectF;
var
  LastFocus: Boolean;
begin
  LastFocus := CanFocus;
...
procedure TGosView.DoGravity;
begin
  Repaint;
end;

procedure TGosView.DoInVisibleChange;
begin
...
procedure TGosView.DoLayoutChanged(Sender: TObject);
begin
  HandleSizeChanged;
end;

procedure TGosView.DoLinkClick(const Text, URL: string);
begin
end;

procedure TGosView.DoMatrixChanged(Sender: TObject);
begin
...
procedure TGosView.DoMaxSizeChange;
begin
  HandleSizeChanged;
end;

procedure TGosView.DoMinSizeChange;
begin
  HandleSizeChanged;
end;

procedure TGosView.DoMouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  if (csDesigning in ComponentState) or FInVisible then Exit;
  if (TMouseButton.mbLeft = Button) and (Clickable or (not (Self is TGosViewGroup))) then begin
...
procedure TGosView.DoMouseEnter;
begin
...
procedure TGosView.DoMouseLeave;
begin
...
procedure TGosView.DoMouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  if (TMouseButton.mbLeft = Button) and (Clickable or (not (Self is TGosViewGroup))) then begin
...
function TGosView.GetCaptureDragForm: Boolean;
begin
  {$IFDEF MSWINDOWS}
...
function TGosView.GetClickable: Boolean;
begin
  Result := HitTest;
end;

function TGosView.GetComponent: TComponent;
begin
  Result := Self;
end;

function TGosView.GetComponentState: TComponentState;
begin
  Result := ComponentState;
end;

function TGosView.GetContentBounds: TRectD;
begin
  Result := TRectD.Empty;
end;

function TGosView.GetDrawState: TViewState;
begin
  Result := FDrawState;
end;

function TGosView.GetGravity: TLayoutGravity;
begin
  Result := FGravity;
end;

function TGosView.GetHeightSize: TViewSize;
begin
  Result := FHeightSize;
end;

function TGosView.GetHScrollBar: TScrollBar;
begin
  Result := nil;
end;

function TGosView.GetInVisible: Boolean;
begin
  Result := FInVisible;
end;

function TGosView.GetIsChecked: Boolean;
begin
  Result := TViewState.Checked in FViewState;
end;

function TGosView.GetLayout: TViewLayout;
begin
...
function TGosView.GetMargin: string;
begin
  Result := GetBoundsFloat(Margins);
end;

function TGosView.GetMaxHeight: Single;
begin
  Result := FMaxHeight;
end;

function TGosView.GetMaxWidth: Single;
begin
  Result := FMaxWidth;
end;

function TGosView.GetMinHeight: Single;
begin
  Result := FMinHeight;
end;

function TGosView.GetMinWidth: Single;
begin
  Result := FMinWidth;
end;

class function TGosView.GetNavigationBarHeight: Single;
begin
  Result := NavigationBarHeight;
end;

function TGosView.GetOpacity: Single;
begin
  Result := AbsoluteOpacity;
end;

function TGosView.GetOrientation: TOrientation;
begin
  Result := FOrientation;
end;

function TGosView.GetPaddings: string;
begin
  Result := GetBoundsFloat(Padding);
end;

function TGosView.GetParentControl: TControl;
begin
  Result := ParentControl;
end;

function TGosView.GetParentForm: TCustomForm;
var
  P: TFmxObject;
begin
  Result := nil;
...
function TGosView.GetParentMaxHeight: Single;
begin
  if FMaxHeight > 0 then
    Result := FMaxHeight
  else begin
    if Assigned(ParentView) then begin
      Result := TGosView(Parent).GetParentMaxHeight;
      if Result > 0 then
...
function TGosView.GetParentMaxWidth: Single;
begin
  if FMaxWidth > 0 then
    Result := FMaxWidth
  else begin
    if Assigned(ParentView) then begin
      Result := TGosView(Parent).GetParentMaxWidth;
      if Result > 0 then
...
function TGosView.GetParentView: IGosViewGroup;
begin
  Supports(Parent, IGosViewGroup, Result);
end;

function TGosView.GetPosition: TPosition;
begin
  Result := Position;
end;

function TGosView.GetScrollSmallChangeFraction: Single;
begin
  Result := SmallChangeFraction;
end;

class function TGosView.GetStatusHeight: Single;
begin
  Result := StatusHeight;
end;

function TGosView.GetViewBackground: TGosDrawable;
begin
...
function TGosView.GetViewRect: TRectF;
begin
  Result := RectF(Padding.Left, Padding.Top,
    Width - Padding.Right + Padding.Left, Height - Padding.Bottom + Padding.Top);
end;

function TGosView.GetViewRectD: TRectD;
begin
  Result := RectD(Padding.Left, Padding.Top,
    Width - Padding.Right + Padding.Left, Height - Padding.Bottom + Padding.Top);
end;

function TGosView.GetViewStates: TViewStates;
begin
  Result := FViewState;
end;

function TGosView.GetVScrollBar: TScrollBar;
begin
  Result := nil;
end;

function TGosView.GetWeight: Single;
begin
  Result := FWeight;
end;

function TGosView.GetWidthSize: TViewSize;
begin
  Result := FWidthSize;
end;

class procedure TGosView.SetRttiObject(Instance: TObject; const Name: string;
  const Value: TObject);
var
  FType: TRttiType;
  FFiled: TRttiField;
...
class procedure TGosView.SetRttiValue(Instance: TObject; const Name: string; const Value: TValue);
var
  FType: TRttiType;
  FFiled: TRttiField;
  FContext: TRttiContext;
...
class procedure TGosView.SetRttiValue<T>(Instance: TObject; const Name: string;
  const Value: T);
var
  FType: TRttiType;
  FFiled: TRttiField;
...
class function TGosView.GetRttiValue(Instance: TObject; const Name: string): TValue;
var
  FType: TRttiType;
  FFiled: TRttiField;
  FContext: TRttiContext;
...
class function TGosView.GetRttiValue<T>(Instance: TObject; const Name: string): T;
var
  FType: TRttiType;
  FFiled: TRttiField;
  FContext: TRttiContext;
...
function TGosView.GetRealDrawState: TViewState;
begin
  if FViewState = [] then
...
class function TGosView.GetRttiObject(Instance: TObject; const Name: string): TObject;
var
  V: TValue;
begin
...
procedure TGosView.HandleSizeChanged;
begin
...
procedure TGosView.HitTestChanged;
begin
...
procedure TGosView.ImagesChanged;
begin
  Repaint;
end;

procedure TGosView.IncChildState(State: TViewState);
var
  I: Integer;
  View: IGosView;
begin
  ...
  for I := 0 to Controls.Count - 1 do begin
    if (State = TViewState.Pressed) and Controls.Items[I].HitTest then
      Continue;
    if Supports(Controls.Items[I], IGosView, View) then begin
...
procedure TGosView.IncViewState(const State: TViewState);
begin
...
procedure TGosView.InitScrollbar;
begin
end;

procedure TGosView.InternalAlign;
begin
end;

procedure TGosView.Invalidate;
begin
...
{$IFDEF ANDROID}
class function TGosView.AppContext: JContext;
begin
...
class procedure TGosView.InvokeMethod(Instance: TObject; const Name: string;
...
class function TGosView.InvokeMethod<T>(Instance: TObject; const Name: string;
...
{$IFDEF ANDROID}
class procedure TGosView.InitAudioManager();
var
...
class procedure TGosView.InitFontGlyphs();
//var
...
function TGosView.IsActivated: Boolean;
begin
  Result := TViewState.Activated in FViewState;
end;

function TGosView.IsAdjustLayout: Boolean;
begin
  Result := True;
end;

function TGosView.IsAutoSize: Boolean;
begin
  Result := False;
end;

function TGosView.IsDesignerControl(Control: TControl): Boolean;
begin
...
function TGosView.IsDrawing: Boolean;
begin
  Result := FDrawing;
end;

function TGosView.IsHovered: Boolean;
begin
  Result := TViewState.Hovered in FViewState;
end;

function TGosView.IsPressed: Boolean;
begin
  Result := TViewState.Pressed in FViewState;
end;

function TGosView.GetAbsoluteInVisible: Boolean;
var
  PV: IGosViewGroup;
begin
...
function TGosView.GetAdjustViewBounds: Boolean;
begin
  Result := FAdjustViewBounds;
end;

function TGosView.GetAniCalculations: TScrollCalculations;
begin
  Result := nil;
end;

function TGosView.GetBackground: TGosDrawable;
begin
  Result := FBackground;
end;

function TGosView.GetBadgeView: IViewBadge;
begin
  Result := FBadgeView;
end;

procedure TGosView.Loaded;
begin
  inherited Loaded;
end;

procedure TGosView.MouseClick(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  {$IFDEF POSIX}
...
procedure TGosView.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
  {$IFDEF POSIX}
  FDownUpOffset := Y;
...
procedure TGosView.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  inherited MouseUp(Button, Shift, X, Y);
  DoMouseUp(Button, Shift, X, Y);
end;

procedure TGosView.DoOrientation;
begin
end;

procedure TGosView.DoRecalcSize(var AWidth, AHeight: Single);
begin
end;

procedure TGosView.DoResize;
begin
  Resize;
end;

procedure TGosView.DoSetScrollBarValue(Scroll: TScrollBar; const Value, ViewportSize: Double);
begin
...
function TGosView.DoSetSize(const ASize: TControlSize;
...
procedure TGosView.DoWeight;
begin
  HandleSizeChanged;
end;

procedure TGosView.EnabledChanged;
begin
...
class function TGosView.ExistRttiValue(Instance: TObject;
...
function TGosView.FindAndCloneStyleResource<T>(const AStyleLookup: string;
...
function TGosView.FindStyleResource<T>(const AStyleLookup: string;
  var AResource: T): Boolean;
var
  StyleObject: TFmxObject;
begin
...
procedure TGosView.FreeScrollbar;
begin
end;

procedure TGosView.Paint;
begin
...
procedure TGosView.PlaySoundEffect(ASoundConstant: Integer);
{$IFDEF ANDROID}
var
  RingerMode: Integer;
begin
...
function TGosView.PointInObject(X, Y: Single): Boolean;
begin
...
procedure TGosView.ReadState(Reader: TReader);
begin
  inherited ReadState(Reader);
end;

procedure TGosView.RecalcInVisible;
var
  I: Integer;
  Item: TControl;
begin
  ...
  for I := 0 to ControlsCount - 1 do begin
    Item := Controls[I];
    if Item is TGosView then
      (Item as TGosView).RecalcInVisible;
  end;
end;

procedure TGosView.SetAdjustViewBounds(const Value: Boolean);
begin
...
procedure TGosView.SetBackground(const Value: TBitmap);
begin
  Background.SetBitmap(TViewState.None, Value);
end;

procedure TGosView.SetBackground(const Value: TBrushBitmap);
begin
  Background.SetBitmap(TViewState.None, Value);
end;

procedure TGosView.SetBackgroundBase(const Value: TGosDrawable);
begin
  SetBackground(Value);
end;

procedure TGosView.SetBadgeView(const Value: IViewBadge);
begin
  if Assigned(Self) then
    FBadgeView := Value;
end;

procedure TGosView.SetBackground(const Value: TAlphaColor);
begin
  Background.SetColor(TViewState.None, Value);
end;

procedure TGosView.SetBackground(const Value: TGradient);
begin
  Background.SetGradient(TViewState.None, Value);
end;

procedure TGosView.SetBackground(const Value: TGosDrawable);
begin
...
procedure TGosView.SetCaptureDragForm(const Value: Boolean);
begin
  {$IFDEF MSWINDOWS}
...
procedure TGosView.SetClickable(const Value: Boolean);
begin
  HitTest := Value;
end;

procedure TGosView.SetDisableMouseWheel(const Value: Boolean);
begin
...
procedure TGosView.SetGravity(const Value: TLayoutGravity);
begin
...
procedure TGosView.SetHeightSize(const Value: TViewSize);
begin
...
procedure TGosView.SetInVisible(const Value: Boolean);
begin
...
procedure TGosView.SetIsChecked(const Value: Boolean);
begin
...
procedure TGosView.SetLayout(const Value: TViewLayout);
begin
  if not AllowUseLayout then
    Exit;
...
procedure TGosView.SetMargin(const Value: string);
var V: Single;
begin
...
procedure TGosView.SetMaxHeight(const Value: Single);
begin
...
procedure TGosView.SetMaxWidth(const Value: Single);
begin
...
procedure TGosView.SetMinHeight(const Value: Single);
begin
...
procedure TGosView.SetMinWidth(const Value: Single);
begin
...
procedure TGosView.SetOrientation(const Value: TOrientation);
begin
...
procedure TGosView.SetPaddings(const Value: string);
var
  V: Single;
begin
  if Assigned(Padding) and GetFloatValue(Value, V) then
...
procedure TGosView.SetScrollbar(const Value: TViewScroll);
begin
...
procedure TGosView.SetTempMaxHeight(const Value: Single);
begin
  if FMaxHeight <> Value then begin
...
procedure TGosView.SetTempMaxWidth(const Value: Single);
begin
  if FMaxWidth <> Value then begin
    if Value > 0 then begin
...
procedure TGosView.SetViewStates(const Value: TViewStates);
begin
  FViewState := Value;
end;

procedure TGosView.SetWeight(const Value: Single);
begin
...
procedure TGosView.SetWidthSize(const Value: TViewSize);
begin
...
procedure TGosView.StartScrolling;
begin
  if Scene <> nil then
    Scene.ChangeScrollingState(Self, True);
end;

procedure TGosView.StartTriggerAnimation(const AInstance: TFmxObject;
  const ATrigger: string);
begin
  // inherited; disable all effect
end;

procedure TGosView.StartTriggerAnimationWait(const AInstance: TFmxObject;
  const ATrigger: string);
begin
  // inherited; disable all effect
end;

procedure TGosView.StartWindowDrag;
var
  F: TCustomForm;
begin
  F := ParentForm;
...
procedure TGosView.StopScrolling;
begin
  if Scene <> nil then
    Scene.ChangeScrollingState(nil, False);
end;

procedure TGosView.UpdateHScrollBar(const Value, ViewportSize: Double);
var
  AScroll: TScrollBar;
begin
  AScroll := HScrollBar;
...
procedure TGosView.UpdateVScrollBar(const Value, ViewportSize: Double);
var
  AScroll: TScrollBar;
begin
  AScroll := VScrollBar;
...
procedure TGosView.VisibleChanged;
begin
...
function TGosViewGroup.AddView(View: TGosView): Integer;
begin
  Result := Controls.Add(View);
end;

constructor TGosViewGroup.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  SetAcceptsControls(True);
end;

destructor TGosViewGroup.Destroy;
begin
  inherited Destroy;
end;

procedure TGosViewGroup.DoAddObject(const AObject: TFmxObject);
begin
  inherited DoAddObject(AObject);
  Realign;
end;

procedure TGosViewGroup.DoGravity;
begin
  //inherited DoGravity;
  Realign;
end;

procedure TGosViewGroup.DoLayoutChanged(Sender: TObject);
begin
  inherited DoLayoutChanged(Sender);
  Realign;
end;

procedure TGosViewGroup.DoMaxSizeChange;
begin
...
procedure TGosViewGroup.DoMinSizeChange;
begin
...
// 是否要调整组件的宽度或高度
function TGosViewGroup.IsAdjustSize(View: IGosView; Align: TAlignLayout;
  AParentOrientation: TOrientation): Boolean;
begin
  if Assigned(View) then begin
    // 实现了 IGosView 接口
...
procedure TGosViewGroup.Loaded;
begin
  inherited Loaded;
...
function TGosViewGroup.RemoveView(View: TGosView): Integer;
begin
  Result := Controls.Remove(View);
end;

procedure TGosViewGroup.Resize;
begin
...
begin
  ...
  if ((W > 0) and (H > 0)) or (CtrlCount > 0) then begin
    ...
    for I := 0 to CtrlCount - 1 do begin
      ...
      // 得到组件IGosView接口，及是否启用最大最小大小限制
      View := nil;
      if (Supports(Control, IGosView, View)) then begin
...
begin
  ...
  for I := ControlsCount - 1 downto 0 do begin
    ...
    if (AControl = nil) then begin
      View := nil;
      Supports(Control, IGosView, View);
...
function TLinearLayout.GetLastWeightView: TGosView;
var
  I: Integer;
  Control: TControl;
  View: IGosView;
begin
  ...
  for I := ControlsCount - 1 downto 0 do begin
    Control := Controls[I];
    if (not Control.Visible) then Continue;
    if (Supports(Control, IGosView, View)) and (View.GetWeight > 0) then begin
      Result := Control as TGosView;
      Break;
    end
  end;
end;
...
begin
  ...
  for I := 0 to ControlsCount - 1 do begin
    ...
    if IsDesignerControl(Control) then Continue;
    {$ENDIF}
    if (Supports(Control, IGosView, View)) and (View.GetWeight > 0) then
  ...
  if (W > 0) and (H > 0) then begin
    ...
    try
      for I := 0 to FViewList.Count - 1 do begin
        View := FViewList[I];
        if not Supports(View, IGosView, LView) then Continue;
...
begin
  if not (csDestroying in ComponentState) then begin
    ...
    for I := 0 to ControlsCount - 1 do begin
      Item := Controls[I];
      if Supports(Item, IGosView, View) then begin
        Layout := View.Layout;
...
begin
  ...
  if not (Supports(Control, IGosView, View)) then begin
...
begin
  ...
  if (csDesigning in AOwner.ComponentState) then begin
    FIsSizeChange := True;
    if TGosView(AOwner).SupportsPlatformService(IFMXDefaultPropertyValueService, DefaultValueService) then
    begin
...
begin
  if Assigned(FOwner) then
    Result := TGosView(FOwner).FillTextFlags
  else Result := [];
end;
...
procedure TTextSettings.SetColor(const Value: TGosViewColor);
begin
  FColor.Assign(Value);
end;
...
constructor TViewImageLink.Create(AOwner: TGosDrawableIcon);
var
  LGlyph: IGlyph;
...
constructor TDrawableBorder.Create(View: IGosView; const ADefaultKind: TViewBrushKind;
  const ADefaultColor: TAlphaColor);
begin
  inherited Create(View, ADefaultKind, ADefaultColor);
end;
...
begin
  ...
  FBrush.OnChanged := DoGradientChanged;
  FColor := TGosViewColor.Create(TAlphaColorRec.Null);
...
procedure TViewBorder.SetColor(const Value: TGosViewColor);
begin
  FColor.Assign(Value);
end;
...
begin
  Result := TGosView.GetRttiValue<TPointD>(Self, 'FDownPoint');
end;
...
begin
  if not (AOwner is TGosView) then
    raise EArgumentException.Create('Argument Invalid.');
  inherited Create(AOwner);
  FScrollView := TGosView(AOwner);
end;
...
constructor TGosDrawableBrush.Create(AOwner: TComponent);
begin
...
procedure TGosDrawableBrush.CreateBrush(var Value: TBrush);
begin
  if Assigned(Value) then
    FreeAndNil(Value);
...
destructor TGosDrawableBrush.Destroy;
begin
  FreeAndNil(FBrush);
  FImageLink.DisposeOf;
  inherited Destroy;
...
procedure TGosDrawableBrush.DoChange(Sender: TObject);
begin
  if Assigned(FOnChanged) then
    FOnChanged(Sender);
end;

procedure TGosDrawableBrush.Draw(Canvas: TCanvas; const R: TRectF;
  const XRadius, YRadius: Single; const ACorners: TCorners;
...
begin
  ...
  if (Ord(FBrush.Kind) = Ord(TViewBrushKind.Patch9Bitmap)) and (FBrush is TViewBrush) then begin
    TGosDrawableBase.FillRect9Patch(Canvas, R, XRadius, YRadius, ACorners, AOpacity, TViewBrush(FBrush), ACornerType);
  end else
...
function TGosDrawableBrush.GetBrush: TBrush;
begin
...
function TGosDrawableBrush.GetComponent: TComponent;
begin
  Result := Self;
end;

function TGosDrawableBrush.GetImageIndex: TImageIndex;
begin
  Result := FImageLink.ImageIndex;
end;

function TGosDrawableBrush.GetImageIndexEx: TImageIndex;
begin
  Result := TViewImagesBrush(Brush).ImageIndex;
end;

function TGosDrawableBrush.GetImageList: TBaseImageList;
begin
  Result := GetImages;
end;

function TGosDrawableBrush.GetImages: TCustomImageList;
begin
  if Assigned(FImageLink.Images) then
    Result := TCustomImageList(FImageLink.Images)
  else
...
function TGosDrawableBrush.GetIsEmpty: Boolean;
begin
...
procedure TGosDrawableBrush.ImagesChanged;
begin
  DoChange(Self);
end;

procedure TGosDrawableBrush.SetBrush(const Value: TBrush);
begin
  if (Value = nil) then begin
    FreeAndNil(FBrush);
  end else begin
...
procedure TGosDrawableBrush.SetImageIndex(const Value: TImageIndex);
begin
  FImageLink.ImageIndex := Value;
end;

procedure TGosDrawableBrush.SetImageIndexEx(const Value: TImageIndex);
begin
  TViewImagesBrush(Brush).ImageIndex := Value;
end;

procedure TGosDrawableBrush.SetImageList(const Value: TBaseImageList);
begin
  ValidateInheritance(Value, TCustomImageList);
  SetImages(TCustomImageList(Value));
end;

procedure TGosDrawableBrush.SetImages(const Value: TCustomImageList);
begin
  FImageLink.Images := Value;
end;
...
procedure TViewHtmlText.CalcTextSize(Canvas: TCanvas;
  TextSet: uGosBase.TTextSettings; const R: TRectF; var ASize: TSizeF);
begin
...
procedure TViewHtmlText.Draw(Canvas: TCanvas; TextSet: uGosBase.TTextSettings;
...
procedure TViewHtmlText.MouseDown(Sender: TGosView; Button: TMouseButton; Shift: TShiftState; X,
  Y: Single);
begin
end;

procedure TViewHtmlText.MouseLeave(Sender: TGosView);
begin
  FLinkHot := -1;
  Sender.Cursor := FDefaultCursor;
end;

procedure TViewHtmlText.MouseMove(Sender: TGosView; X, Y: Single);
var
  I: Integer;
begin
  I := PointInLink(X, Y);
...
procedure TViewHtmlText.MouseUp(Sender: TGosView; Button: TMouseButton;
...
initialization
  ...
  {$IFDEF ANDROID}
  TGosView.InitAudioManager();
  TGosView.InitFontGlyphs();
